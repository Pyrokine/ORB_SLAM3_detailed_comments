...
//      2. 对于行列数不同的矩阵，例如 3*4 或者 4*3 矩阵只有3个奇异向量，计算的时候如果是 Thin 那么得出的UV矩阵列数只能是 3，如果是 full 那么就是 4
//      3. thin会损失一部分数据，但是会加快计算，对于大型矩阵解算方程时，可以用 thin 加速得到结果
 ...
 * @return Jr
 */
    [[maybe_unused]] Eigen::Matrix3f RightJacobianSO3(const Eigen::Vector3f &v) {
 ...
 * @return Jr^-1
 */
    [[maybe_unused]] Eigen::Matrix3f InverseRightJacobianSO3(const Eigen::Vector3f &v) {
 ...
 */
    Preintegrated::Preintegrated(const Bias &b_, [[maybe_unused]] const Calib &calib) {
        Nga = calib.Cov;
...
//      根据 η_ij = A * η_i,j-1 + B_j-1 * η_j-1 中的Ａ矩阵和Ｂ矩阵对速度和位移进行更新
 ...
 * @return dR
 */
    [[maybe_unused]] Eigen::Matrix3f Preintegrated::GetOriginalDeltaRotation() {
        std::unique_lock<std::mutex> lock(mMutex);
        return dR;
    }
 ...
 * @return dV
 */
    [[maybe_unused]] Eigen::Vector3f Preintegrated::GetOriginalDeltaVelocity() {
        std::unique_lock<std::mutex> lock(mMutex);
        return dV;
    }
 ...
 * @return dP
 */
    [[maybe_unused]] Eigen::Vector3f Preintegrated::GetOriginalDeltaPosition() {
        std::unique_lock<std::mutex> lock(mMutex);
        return dP;
    }
 ...
 * @return b
 */
    [[maybe_unused]] Bias Preintegrated::GetOriginalBias() {
        std::unique_lock<std::mutex> lock(mMutex);
        return b;
    }
 ...
 * @return db
 */
    [[maybe_unused]] Eigen::Matrix<float, 6, 1> Preintegrated::GetDeltaBias() {
        std::unique_lock<std::mutex> lock(mMutex);
        return db;
    }
