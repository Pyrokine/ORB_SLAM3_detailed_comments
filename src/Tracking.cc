 ...
 */
    Tracking::Tracking(System *pSys, ORBVocabulary *pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Atlas *pAtlas,
            ...
            : mState(NO_IMAGES_YET), mSensor(sensor), mTrackedFr(0), mbStep(false), mbOnlyTracking(false), mbMapUpdated(false), mbVO(false),
              mpORBVocabulary(pVoc), mpKeyFrameDB(pKFDB), mbReadyToInitialize(false), mpSystem(pSys), mpViewer(nullptr), bStepByStep(false),
              mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpAtlas(pAtlas), mnLastRelocFrameId(0), time_recently_lost(5.0),
              mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr), mpLastKeyFrame(static_cast<KeyFrame *>(nullptr)) {
//      Step 1 从配置文件中加载相机参数
        if (settings) {
...
//          Distortion parameters
            ...
            if (!node.empty() && node.isReal()) {
                mDistCoef.at<float>(0) = float(node.real());
            } else {
            ...
            if (!node.empty() && node.isReal()) {
                mDistCoef.at<float>(1) = float(node.real());
            } else {
            ...
            if (!node.empty() && node.isReal()) {
                mDistCoef.at<float>(2) = float(node.real());
            } else {
            ...
            if (!node.empty() && node.isReal()) {
                mDistCoef.at<float>(3) = float(node.real());
            } else {
            ...
            if (!node.empty() && node.isReal()) {
                mDistCoef.resize(5);
                mDistCoef.at<float>(4) = float(node.real());
            }

            node = fSettings["Camera.imageScale"];
            if (!node.empty() && node.isReal()) {
                mImageScale = float(node.real());
            }
...
//                      K matrix parameters must be scaled.
                        ...
                        leftLappingBegin = int(float(leftLappingBegin) * mImageScale);
                        leftLappingEnd = int(float(leftLappingEnd) * mImageScale);
                        rightLappingBegin = int(float(rightLappingBegin) * mImageScale);
                        rightLappingEnd = int(float(rightLappingEnd) * mImageScale);
                    }
...
//      Max/Min Frames to insert keyframes and to check relocalization
        ...
        if (mSensor == System::RGBD || mSensor == System::IMU_RGBD) {
            ...
            if (!node.empty() && node.isReal()) {
                mDepthMapFactor = float(node.real());
                if (fabs(mDepthMapFactor) < 1e-5)
                    mDepthMapFactor = 1;
                else
                    mDepthMapFactor = 1.0f / mDepthMapFactor;
 ...
 */
    bool Tracking::ParseIMUParamFile(cv::FileStorage &fSettings) {
        ...
        if (!node.empty() && node.isInt()) {
            mImuFreq = float(node.operator int());
...
//  一步一步进行
    ...
    [[maybe_unused]] bool Tracking::GetStepByStep() const {
        return bStepByStep;
    }
...
//          第一帧数据但不是最后两帧,imu总帧数大于2
            if ((i == 0) && (i < (n - 1))) {
//              获取相邻两段imu的时间间隔
                auto tab = float(mvImuFromLastFrame[i + 1].t - mvImuFromLastFrame[i].t);
//              获取当前imu到上一帧的时间间隔
                auto tini = float(mvImuFromLastFrame[i].t - mCurrentFrame.mpPrevFrame->mTimeStamp);
...
//              计算过程类似加速度
                ...
                tstep = float(mvImuFromLastFrame[i + 1].t - mCurrentFrame.mpPrevFrame->mTimeStamp);
            } else if (i < (n - 1)) {
//              中间的数据不存在帧的干扰，正常计算
                acc = (mvImuFromLastFrame[i].a + mvImuFromLastFrame[i + 1].a) * 0.5f;
                angVel = (mvImuFromLastFrame[i].w + mvImuFromLastFrame[i + 1].w) * 0.5f;
                tstep = float(mvImuFromLastFrame[i + 1].t - mvImuFromLastFrame[i].t);
            }
//          直到倒数第二个imu时刻时，计算过程跟第一时刻类似，都需要考虑帧与imu时刻的关系
            else if ((i > 0) && (i == (n - 1))) {
                auto tab = float(mvImuFromLastFrame[i + 1].t - mvImuFromLastFrame[i].t);
                auto tend = float(mvImuFromLastFrame[i + 1].t - mCurrentFrame.mTimeStamp);
                acc = (mvImuFromLastFrame[i].a + mvImuFromLastFrame[i + 1].a -
                       (mvImuFromLastFrame[i + 1].a - mvImuFromLastFrame[i].a) * (tend / tab)) * 0.5f;
                angVel = (mvImuFromLastFrame[i].w + mvImuFromLastFrame[i + 1].w -
                          (mvImuFromLastFrame[i + 1].w - mvImuFromLastFrame[i].w) * (tend / tab)) * 0.5f;
                tstep = float(mCurrentFrame.mTimeStamp - mvImuFromLastFrame[i].t);
            }
//          就两个数据时使用第一个时刻的，这种情况应该没有吧，，回头应该试试看
            else if ((i == 0) && (i == (n - 1))) {
                acc = mvImuFromLastFrame[i].a;
                angVel = mvImuFromLastFrame[i].w;
                tstep = float(mCurrentFrame.mTimeStamp - mCurrentFrame.mpPrevFrame->mTimeStamp);
            }
//          Step 3.依次进行预积分计算
...
//                          将恒速模型跟踪结果暂存到这几个变量中，因为后面重定位会改变这些变量
                            ...
                            vbOutMM = mCurrentFrame.mvbExterior;
...
//                          恒速模型成功、重定位失败，重新使用之前暂存的恒速模型结果
                            ...
                            mCurrentFrame.mvbExterior = vbOutMM;
...
//                                  如果这个特征点形成了地图点,并且也不是外点的时候
                                    if (mCurrentFrame.mvpMapPoints[i] && !mCurrentFrame.mvbExterior[i]) {
//                                      增加被找到次数
...
//                TODO check this situation
                ...
                auto *pF = new Frame(mCurrentFrame);
                ...
                for (int i = 0; i < mCurrentFrame.N; ++i) {
                    ...
                    if (pMP)
                        if (pMP->Observations() < 1) {
                            mCurrentFrame.mvbExterior[i] = false;
...
//                      通过反投影得到该特征点的世界坐标系下3D坐标
                        Eigen::Vector3f x3D;
                        mCurrentFrame.UnprojectedStereo(i, x3D);
...
//          a.表示该MapPoint可以被哪个KeyFrame的哪个特征点观测到
            pMP->AddObservation(pKFini, int(i));
...
//          mvIniMatches[i]是初始化当前帧中的特征点的序号
            ...
            mCurrentFrame.mvbExterior[mvIniMatches[i]] = false;
...
//              如果对应到的某个特征点是外点
                if (mCurrentFrame.mvbExterior[i]) {
//                  清除它在当前帧中存在过的痕迹
                    MapPoint *pMP = mCurrentFrame.mvpMapPoints[i];
...
//      ref_keyframe 到 last frame 的位姿变换
...
//              反投影到世界坐标系中
                ...
                if (mLastFrame.Nleft == -1) {
                    mLastFrame.UnprojectedStereo(i, x3D);
                } else {
                    x3D = mLastFrame.UnprojectedStereoFishEye(i);
                }

//              加入上一帧的地图点中
                auto *pNewMP = new MapPoint(x3D, mpAtlas->GetCurrentMap(), &mLastFrame, i);
...
//      Step 3：用上一帧地图点进行投影匹配，如果匹配点不够，则扩大搜索半径再来一次
        int nMatches = matcher.SearchByProjection(mCurrentFrame, mLastFrame, float(th), mSensor == System::MONOCULAR || mSensor == System::IMU_MONOCULAR);
...
//      如果匹配点太少，则扩大搜索半径再来一次
        if (nMatches < 20) {
            ...
            nMatches = matcher.SearchByProjection(mCurrentFrame, mLastFrame, 2.0f * float(th), mSensor == System::MONOCULAR || mSensor == System::IMU_MONOCULAR);
...
//                if(!mbMapUpdated && mState == OK)// && (mnMatchesInners>30))
//              mbMapUpdated变化见Tracking::PredictStateIMU()
//              未更新地图
                if (!mbMapUpdated /*&& (mnMatchesInners>30)*/) {
                    Verbose::PrintMess("TLM: PoseInertialOptimizationLastFrame ", Verbose::VERBOSITY_DEBUG);
//                  使用上一普通帧以及当前帧的视觉信息和IMU信息联合优化当前帧位姿、速度和IMU零偏
                    inners = Optimizer::PoseInertialOptimizationLastFrame(&mCurrentFrame);  // , !mpLastKeyFrame->GetMap()->GetInertialBA1());
                } else {
                    Verbose::PrintMess("TLM: PoseInertialOptimizationLastKeyFrame ", Verbose::VERBOSITY_DEBUG);
//                  使用上一关键帧以及当前帧的视觉信息和IMU信息联合优化当前帧位姿、速度和IMU零偏
                    inners = Optimizer::PoseInertialOptimizationLastKeyFrame(&mCurrentFrame);  // , !mpLastKeyFrame->GetMap()->GetInertialBA1());
                }
            }
        }
//      查看内外点数目，调试用
...
//      More restrictive if there was a relocalization recently
        mpLocalMapper->mnMatchesInners = mnMatchesInners;
//      Step 5：根据跟踪匹配数目及重定位情况决定是否跟踪成功
//      如果最近刚刚发生了重定位,那么至少成功匹配50个点才认为是成功跟踪
        if (mCurrentFrame.mnId < mnLastRelocFrameId + mMaxFrames && mnMatchesInners < 50)
            return false;

//      RECENTLY_LOST状态下，至少成功跟踪10个才算成功
        if ((mnMatchesInners > 10) && (mState == RECENTLY_LOST))
            return true;

//      单目IMU模式下做完初始化至少成功跟踪15个才算成功，没做初始化需要50个
        if (mSensor == System::IMU_MONOCULAR) {
            if ((mnMatchesInners < 15 && mpAtlas->isImuInitialized()) || (mnMatchesInners < 50 && !mpAtlas->isImuInitialized())) {
                return false;
            } else
                return true;
        } else if (mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD) {
            if (mnMatchesInners < 15) {
                return false;
            } else
                return true;
        } else {
//          以上情况都不满足，只要跟踪的地图点大于30个就认为成功了
            if (mnMatchesInners < 30)
                return false;
            else
                return true;
        }
...
//          如果是IMU模式，当前帧距离上一关键帧时间戳超过0.25s，则说明需要插入关键帧，不再进行后续判断
            if (
                    (mSensor == System::IMU_MONOCULAR && (mCurrentFrame.mTimeStamp - mpLastKeyFrame->mTimeStamp) >= 0.25) ||
                    ((mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD) && (mCurrentFrame.mTimeStamp - mpLastKeyFrame->mTimeStamp) >= 0.25)
                )
                return true;
            else
                return false;
...
//            if(mSensor == System::MONOCULAR) {
//                std::cout << "NeedNewKeyFrame: local map stopped" << std::endl;
//            }
            return false;
        }

//      获取当前地图中的关键帧数目
        const int nKFs = int(mpAtlas->KeyFramesInMap());
...
//      单目+IMU情况下如果，匹配内点数目超过350，插入关键帧的频率可以适当降低
        if (mSensor == System::IMU_MONOCULAR) {
            if (mnMatchesInners > 350)// Points tracked from the local map
                thRefRatio = 0.75f;
            else
                thRefRatio = 0.90f;
        }
...
//      Step 7.4：在双目，RGB-D的情况下当前帧跟踪到的点比参考关键帧的0.25倍还少，或者满足bNeedToInsertClose
        const bool c1c = mSensor != System::MONOCULAR && mSensor != System::IMU_MONOCULAR && mSensor != System::IMU_STEREO && mSensor != System::IMU_RGBD && (mnMatchesInners < nRefMatches * 0.25 || bNeedToInsertClose);
//      Condition 2: Few tracked points compared to reference keyframe. Lots of visual odometry compared to map matches.
//      Step 7.5：和参考帧相比当前跟踪到的点太少 或者满足bNeedToInsertClose；同时跟踪到的内点还不能太少
        const bool c2 = (((float(mnMatchesInners) < float(nRefMatches) * thRefRatio || bNeedToInsertClose)) && mnMatchesInners > 15);
...
//      新增的条件c4：单目+IMU模式下，当前帧匹配内点数在15~75之间或者是RECENTLY_LOST状态，c4=true
        bool c4 = false;
//        MODIFICATION_2, originally ((((mnMatchesInners<75) && (mnMatchesInners>15)) || mState==RECENTLY_LOST) && ((mSensor == System::IMU_MONOCULAR)))
        if ((((mnMatchesInners < 75) && (mnMatchesInners > 15)) || mState == RECENTLY_LOST) && (mSensor == System::IMU_MONOCULAR))
            c4 = true;
        else
            c4 = false;
...
//                  std::cout << "NeedNewKeyFrame: local map is busy" << std::endl;
//                  对于单目情况,就直接无法插入关键帧了
//                  ? 为什么这里对单目情况的处理不一样?
//                  回答：可能是单目关键帧相对比较密集
                    return false;
...
//                  如果需要就新建地图点，这里的地图点不是临时的，是全局地图中新建地图点，用于跟踪
                    if (bCreateNew) {
                        ...
                        if (mCurrentFrame.Nleft == -1) {
                            ...
                            x3D = mCurrentFrame.UnprojectedStereoFishEye(i);
                        }

                        auto *pNewMP = new MapPoint(x3D, pKF, mpAtlas->GetCurrentMap());
...
//          如果不久前进行过重定位，那么进行一个更加宽泛的搜索，阈值需要增大
            ...
            if (mState == LOST || mState == RECENTLY_LOST)// Lost for less than 1 second
                th = 15;  // 15
//          投影匹配得到更多的匹配关系
            int matches = matcher.SearchByProjection(mCurrentFrame, mvpLocalMapPoints, float(th), mpLocalMapper->mbFarPoints, mpLocalMapper->mThFarPoints);
        }
    }
...
//          ?为什么IMU初始化后用mLastFrame？mLastFrame存储的是上一帧跟踪成功后帧数据。
            for (int i = 0; i < mLastFrame.N; ++i) {
//              Using last frame since current frame has not matches yet
...
//          类型3:将一级共视关键帧的子关键帧作为局部关键帧（将邻居的孩子们拉拢入伙）
            const set<KeyFrame *> spChildren = pKF->GetChildren();
            for (auto pChildKF : spChildren) {
                if (!pChildKF->isBad()) {
...
//      Step 2：找到与当前帧相似的候选关键帧组
        ...
        auto nKFs = int(vpCandidateKFs.size());
...
//      Until we found a camera pose supported by enough inner points
...
//              PnP算法的入口函数
                bool bTcw = pSolver->iterate(5, bNoMore, vbInners, nInners, eigTcw);
...
//                  删除外点对应的地图点,这里直接设为空指针
                    for (int io = 0; io < mCurrentFrame.N; ++io)
                        if (mCurrentFrame.mvbExterior[io])
                            mCurrentFrame.mvpMapPoints[io] = static_cast<MapPoint *>(nullptr);

//                  If few inners, search by projection in a coarse window and optimize again
...
//                          If many inners but still not enough, search by projection again in a narrower window
...
//                  If the pose is supported by enough inners stop RANSACs and continue
 ...
 */
    void Tracking::ResetActiveMap(bool bLocMap) {
        ...
        if (!bLocMap) {
            Verbose::PrintMess("Resetting Local Mapper...", Verbose::VERBOSITY_VERY_VERBOSE);
 ...
 * local mapping中初始化imu中使用，速度的走向（仅在imu模式使用），最开始速度定义于imu初始化时，每个关键帧都根据位移除以时间得到，经过非线性优化保存于KF中.
...
//      每一帧的参考关键帧
        ...
        auto lbL = mlbLost.begin();  // 对应帧是否跟踪丢失
...
//      当前帧是否做了预积分
        ...
        mnFirstImuFrameId = int(mCurrentFrame.mnId);
    }
    ...
    [[maybe_unused]] int Tracking::GetNumberDataset() const {
        return mnNumDataset;
    }

    int Tracking::GetMatchesInners() const {
        return mnMatchesInners;
    }

    [[maybe_unused]] void Tracking::SaveSubTrajectory(const string& strNameFile_frames, const string& strNameFile_kf, const string& strFolder) {
        mpSystem->SaveTrajectoryEuRoC(strFolder + strNameFile_frames);
//        mpSystem->SaveKeyFrameTrajectoryEuRoC(strFolder + strNameFile_kf);
    }

    [[maybe_unused]] void Tracking::SaveSubTrajectory(const string& strNameFile_frames, const string& strNameFile_kf, Map *pMap) {
    ...
    float Tracking::GetImageScale() const {
        return mImageScale;
    }
