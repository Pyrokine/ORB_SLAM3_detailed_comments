...
namespace ORB_SLAM3 {
    ...
    Map::Map()
            : mnMaxKFid(0), mnBigChangeIdx(0), mbImuInitialized(false), mnMapChange(0), mpFirstRegionKF(static_cast<KeyFrame *>(nullptr)),
...
//      1. 剔除一下无效观测
        for (MapPoint *pMPi: mspMapPoints) {
            ...
            if (int(pMPi->GetObservations().size()) == 0) {
                nMPWithoutObs++;
            }
