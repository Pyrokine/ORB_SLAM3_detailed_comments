 ...
 * @return 
 */
    Eigen::Vector3f KannalaBrandt8::unprojectedEig(const cv::Point2f &p2D) {
        cv::Point3f ray = this->unprojected(p2D);
 ...
 * @return 
 */
    cv::Point3f KannalaBrandt8::unprojected(const cv::Point2f &p2D) {
 ...
 * @return 
 */
    Eigen::Matrix<double, 2, 3> KannalaBrandt8::projectedJac(const Eigen::Vector3d &v3D) {
...
//  没用
    bool KannalaBrandt8::matchAndTriangulate(const cv::KeyPoint &kp1, const cv::KeyPoint &kp2, GeometricCamera *pOther, Sophus::SE3f &Tcw1, Sophus::SE3f &Tcw2,
                                             float sigmaLevel1, float sigmaLevel2, Eigen::Vector3f &x3DTriangulated) {
        ...
        cv::Point3f ray1c = this->unprojected(kp1.pt);
        cv::Point3f ray2c = pOther->unprojected(kp2.pt);
...
//      1. 得到对应特征点的归一化平面坐标
        Eigen::Vector3f r1 = this->unprojectedEig(kp1.pt);
        Eigen::Vector3f r2 = pCamera2->unprojectedEig(kp2.pt);
...
//      发现上述矩阵线性相关，所以取前两维，两个点构成了4行的矩阵，就是如下的操作，求出的是4维的结果[X,Y,Z,A]，所以需要除以最后一维使之为1，就成了[X,Y,Z,1]这种齐次形式
    ...
    bool KannalaBrandt8::IsEqual(GeometricCamera *pCam) {
        ...
        auto *pKBCam = (KannalaBrandt8 *) pCam;
        ...
        for (size_t i = 0; i < size(); ++i) {
            if (abs(mvParameters[i] - pKBCam->getParameter(int(i))) > 1e-6) {
