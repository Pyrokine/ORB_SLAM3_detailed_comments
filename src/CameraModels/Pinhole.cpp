 ...
 * @return 归一化坐标
 */
    Eigen::Vector3f Pinhole::unprojectedEig(const cv::Point2f &p2D) {
 ...
 * @return 归一化坐标
 */
    cv::Point3f Pinhole::unprojected(const cv::Point2f &p2D) {
        return cv::Point3f(
 ...
 * @return 
 */
    Eigen::Matrix<double, 2, 3> Pinhole::projectedJac(const Eigen::Vector3d &v3D) {
...
//      d = |a*u2 + b*v2 + c| / sqrt(a^2 + b^2)
    ...
    bool Pinhole::IsEqual(GeometricCamera *pCam) {
        ...
        for (size_t i = 0; i < size(); ++i) {
            if (abs(mvParameters[i] - pPinholeCam->getParameter(int(i))) > 1e-6) {
