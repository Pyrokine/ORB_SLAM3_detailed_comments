 ...
 */
    [[maybe_unused]] Initializer::Initializer(const Frame &ReferenceFrame, float sigma, int iterations) {
...
//      thread方法比较特殊，在传递引用的时候，外层需要用ref来进行引用传递，否则就是浅拷贝
        thread threadH(&Initializer::FindHomography,  // 该线程的主函数
                       ...
                       ref(vbMatchesInliersH),  // 输出，特征点对的Inner标记
...
//      cout << "6" << endl;
        ...
        cv::Mat K = dynamic_cast<Pinhole *>(mpCamera)->toK();
...
//      Number of putative matches
//      匹配的特征点对总数
        const auto N = int(vbMatchesInliers.size());
...
//      Step 4.1 使用同样的匹配点分别检查四组解，记录当前计算的3D点在摄像头前方且投影误差小于阈值的个数，记为有效3D点个数
        int nGood1 = CheckRT(R1, t1,  // 当前组解
                             ...
                             4.0f * mSigma2,  // 三角化测量过程中允许的最大重投影误差
...
//      因为det(V)==det(Vt), 所以 s = det(U) * det(Vt)
        auto s = float(cv::determinant(U) * cv::determinant(Vt));
...
//      在ORB SLAM中没有对奇异值 d1 d2 d3按照论文中描述的关系进行分类讨论, 而是直接进行了计算
...
//          调用 Initializer::CheckRT(), 计算good点的数目
            int nGood = CheckRT(vR[i], vt[i],  // 当前组解的旋转矩阵和平移向量
                                mvKeys1, mvKeys2,  // 特征点
                                mvMatches12, vbMatchesInliers,  // 特征匹配关系以及Inner标记
                                K,  // 相机的内参数矩阵
                                vP3Di,  // 存储三角化测量之后的特征点空间坐标的
                                4.0f * mSigma2,  // 三角化过程中允许的最大重投影误差
 ...
 */
    ...
    {
//      原理
//      Triangularization: 已知匹配特征点对{x x'} 和 各自相机矩阵{P P'}, 估计三维点 X
//      x' = P'X  x = PX
//      它们都属于 x = aPX模型
//      |X|
 ...
 * @return int 
 */
    int Initializer::CheckRT(const cv::Mat &R, const cv::Mat &t, const vector<cv::KeyPoint> &vKeys1, const vector<cv::KeyPoint> &vKeys2, const vector<Match> &vMatches12,
...
//          求取模长，其实就是距离
            auto dist1 = float(cv::norm(normal1));

//          同理构造向量PO2
            cv::Mat normal2 = p3dC1 - O2;
//          求模长
            auto dist2 = float(cv::norm(normal2));

//          根据公式：a.*b=|a||b|cos_theta 可以推导出来下面的式子
            auto cosParallax = float(normal1.dot(normal2) / (dist1 * dist2));
...
//          将这个选中的角弧度制转换为角度制
            parallax = acosf(vCosParallax[idx]) * 180.0f / float(CV_PI);
        } else
