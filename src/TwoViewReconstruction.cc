...
//      Fill structures with current key points and matches with reference frame.
...
//      长度与vKeys1，表示对应位置的vKeys1中的点是否有匹配关系
        ...
        const auto N = int(mvMatches12.size());
...
//            Select a minimum set
            for (size_t j = 0; j < 8; ++j) {
                ...
                auto idx = int(vAvailableIndices[randi]);  // 这句不多余，防止重复选择
...
//      Launch threads to compute in parallel a fundamental matrix and a homography
        vector<bool> vbMatchesInnersH, vbMatchesInnersF;
        float SH, SF;
        Eigen::Matrix3f H, F;

//      3. 双线程分别计算
//      加ref为了提供引用
        thread threadH(&TwoViewReconstruction::FindHomography, this, ref(vbMatchesInnersH), ref(SH), ref(H));
        thread threadF(&TwoViewReconstruction::FindFundamental, this, ref(vbMatchesInnersF), ref(SF), ref(F));
...
//      ORB SLAM2这里的值是0.4， TOSEE
        if (RH > 0.50) {  // if (RH > 0.40)
//            cout << "Initialization from Homography" << endl;
            return ReconstructH(vbMatchesInnersH, H, mK, T21, vP3D, vbTriangulated, minParallax, 50);
        } else {  // if (pF_HF > 0.6)
//            cout << "Initialization from Fundamental" << endl;
            return ReconstructF(vbMatchesInnersF, F, mK, T21, vP3D, vbTriangulated, minParallax, 50);
        }
    }

/** 
 * @brief 计算H矩阵，同时计算得分与内点
 * @param vbMatchesInners 经过H矩阵验证，是否为内点，大小为mvMatches12
 * @param score 得分
 * @param H21 1到2的H矩阵
 */
    void TwoViewReconstruction::FindHomography(vector<bool> &vbMatchesInners, float &score, Eigen::Matrix3f &H21) {
//      匹配成功的个数
        const auto N = int(mvMatches12.size());
...
//      Best Results variables
        score = 0.0;
        vbMatchesInners = vector<bool>(N, false);

//      Iteration variables
        vector<cv::Point2f> vPn1i(8);
        vector<cv::Point2f> vPn2i(8);
        Eigen::Matrix3f H21i, H12i;
        vector<bool> vbCurrentInners(N, false);
        float currentScore;
...
//          恢复正常H
            ...
            currentScore = CheckHomography(H21i, H12i, vbCurrentInners, mSigma);  // mSigma默认为1

            if (currentScore > score) {
                H21 = H21i;
                vbMatchesInners = vbCurrentInners;
                score = currentScore;
            }
        }
    }
 ...
 * @param vbInners 经过F矩阵验证，是否为内点，大小为mvMatches12
 * @param score 得分
 * @param F21 1到2的F矩阵
 */
    void TwoViewReconstruction::FindFundamental(vector<bool> &vbInners, float &score, Eigen::Matrix3f &F21) {
//      Number of putative matches
        const auto N = int(vbInners.size());
...
//          FN得到的是基于归一化坐标的F矩阵，必须乘上归一化过程矩阵才是最后的基于像素坐标的F
            ...
            currentScore = CheckFundamental(F21i, vbCurrentInners, mSigma);

            if (currentScore > score) {
                F21 = F21i;
                vbInners = vbCurrentInners;
                score = currentScore;
            }
        }
    }
 ...
 * @param H21 顾名思义
 * @param H12 顾名思义
 * @param vbMatchesInners 匹配是否合法，大小为mvMatches12
 * @param sigma 默认为1
 */
    float TwoViewReconstruction::CheckHomography(const Eigen::Matrix3f &H21, const Eigen::Matrix3f &H12, vector<bool> &vbMatchesInners, float sigma) {
        const auto N = int(mvMatches12.size());
...
//          x1in2 = H21*x1
            ...
            if (bIn)
                vbMatchesInners[i] = true;
            else
                vbMatchesInners[i] = false;
        }
 ...
 * @param sigma 默认为1
 */
    float TwoViewReconstruction::CheckFundamental(const Eigen::Matrix3f &F21, vector<bool> &vbMatchesInners, float sigma) {
        const auto N = int(mvMatches12.size());
 ...
 */
    bool TwoViewReconstruction::ReconstructF(
            vector<bool> &vbMatchesInners, Eigen::Matrix3f &F21, Eigen::Matrix3f &K,
...
//      统计了合法的匹配，后面用于对比重建出的点数
        int N = 0;
        for (auto && vbMatchesInner : vbMatchesInners)
            if (vbMatchesInner)
                N++;
...
//      3. 使用四对结果分别重建
        ...
        int nGood1 = CheckRT(R1, t1, mvKeys1, mvKeys2, mvMatches12, vbMatchesInners, K, vP3D1, 4.0f * mSigma2, vbTriangulated1, parallax1);
        int nGood2 = CheckRT(R2, t1, mvKeys1, mvKeys2, mvMatches12, vbMatchesInners, K, vP3D2, 4.0f * mSigma2, vbTriangulated2, parallax2);
        int nGood3 = CheckRT(R1, t2, mvKeys1, mvKeys2, mvMatches12, vbMatchesInners, K, vP3D3, 4.0f * mSigma2, vbTriangulated3, parallax3);
        int nGood4 = CheckRT(R2, t2, mvKeys1, mvKeys2, mvMatches12, vbMatchesInners, K, vP3D4, 4.0f * mSigma2, vbTriangulated4, parallax4);
//      统计重建出点的数量最大值
 ...
 */
    bool TwoViewReconstruction::ReconstructH(
            vector<bool> &vbMatchesInners, Eigen::Matrix3f &H21, Eigen::Matrix3f &K,
            Sophus::SE3f &T21, vector <cv::Point3f> &vP3D, vector<bool> &vbTriangulated, float minParallax, int minTriangulated) {
//      统计了合法的匹配，后面用于对比重建出的点数
        int N = 0;
        for (auto && vbMatchesInner : vbMatchesInners)
...
//      step4：d'=d2和d'=-d2分别对应8组(R t)，通过恢复3D点并判断是否在相机正前方的方法来确定最优解
        for (size_t i = 0; i < 8; ++i) {
            ...
            int nGood = CheckRT(vR[i], vt[i], mvKeys1, mvKeys2, mvMatches12, vbMatchesInners, K, vP3Di, 4.0f * mSigma2, vbTriangulatedi, parallaxi);
 ...
 * @param vbMatchesInners 匹配关系是否有效
 * @param K 内参
 * @param vP3D 三维点
 * @param th2 误差半径
 ...
 */
    int TwoViewReconstruction::CheckRT(
            ...
            const vector <Match> &vMatches12, vector<bool> &vbMatchesInners,
...
//          将这个选中的角弧度制转换为角度制
            parallax = float(acos(vCosParallax[idx]) * 180 / CV_PI);
        } else
            parallax = 0;
