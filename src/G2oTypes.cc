...
//      This is only for pose graph, we do not care about multi camera
        tcw.resize(1);
        Rcw.resize(1);
        tcb.resize(1);
        Rcb.resize(1);
 ...
 */
    void ImuCamPose::SetParam(
        ...
        Rcw = _Rcw;
        tcw = _tcw;
        const auto num_cams = int(Rbc.size());
        Rcb.resize(num_cams);
        tcb.resize(num_cams);
 ...
 */
    bool VertexPose::read(std::istream &is) {
        ...
        const auto num_cams = int(_estimate.Rbc.size());
 ...
 */
    bool VertexPose::write(std::ostream &os) const {
        ...
        const auto num_cams = int(tcw.size());
        ...
        for (int idx = 0; idx < num_cams; ++idx) {
            ...
            for (size_t i = 0; i < _estimate.pCamera[idx]->size(); ++i) {
                os << _estimate.pCamera[idx]->getParameter(int(i)) << " ";
            }
        }
 ...
 */
    void EdgeMono::linearizeOplus() {
        ...
        const auto *VPoint = dynamic_cast<const g2o::VertexSBAPointXYZ *>(_vertices[0]);
        ...
        const Eigen::Matrix<double, 2, 3> proj_jac = VPose->estimate().pCamera[cam_idx]->projectedJac(Xc);
...
//      差了个负号，因为与proj_jac负号相抵，因此是正确的
        _jacobianOplusXj = proj_jac * Rcb * SE3deriv;  // symbol different because of update mode
    }
 ...
 */
    void EdgeMonoOnlyPose::linearizeOplus() {
        ...
        Eigen::Matrix<double, 2, 3> proj_jac = VPose->estimate().pCamera[cam_idx]->projectedJac(Xc);
        ...
        SE3deriv << 0.0, z, -y, 1.0, 0.0, 0.0,
                -z, 0.0, x, 0.0, 1.0, 0.0,
                y, -x, 0.0, 0.0, 0.0, 1.0;
        _jacobianOplusXi = proj_jac * Rcb * SE3deriv;  // symbol different because of update mode
    }
 ...
 */
    void EdgeStereo::linearizeOplus() {
        ...
        proj_jac.block<2, 3>(0, 0) = VPose->estimate().pCamera[cam_idx]->projectedJac(Xc);
...
//      TODO Maybe Reintegrate inertial measurements when difference between linearization point and current estimate is too big
        const auto *VP1 = dynamic_cast<const VertexPose *>(_vertices[0]);  // 位姿Ti
        const auto *VV1 = dynamic_cast<const VertexVelocity *>(_vertices[1]);  // 速度vi
        const auto *VG1 = dynamic_cast<const VertexGyroBias *>(_vertices[2]);  // 零偏Bgi
        const auto *VA1 = dynamic_cast<const VertexAccBias *>(_vertices[3]);  // 零偏Bai
        const auto *VP2 = dynamic_cast<const VertexPose *>(_vertices[4]);  // 位姿Tj
        const auto *VV2 = dynamic_cast<const VertexVelocity *>(_vertices[5]);  // 速度vj
        const IMU::Bias b1((float)VA1->estimate()[0], (float)VA1->estimate()[1], (float)VA1->estimate()[2], 
                           (float)VG1->estimate()[0], (float)VG1->estimate()[1], (float)VG1->estimate()[2]);
...
//  计算雅克比矩阵
    void EdgeInertial::linearizeOplus() {
        ...
        const auto *VV1 = dynamic_cast<const VertexVelocity *>(_vertices[1]);
        const auto *VG1 = dynamic_cast<const VertexGyroBias *>(_vertices[2]);
        const auto *VA1 = dynamic_cast<const VertexAccBias *>(_vertices[3]);
        const auto *VP2 = dynamic_cast<const VertexPose *>(_vertices[4]);
        const auto *VV2 = dynamic_cast<const VertexVelocity *>(_vertices[5]);
        const IMU::Bias b1((float)VA1->estimate()[0], (float)VA1->estimate()[1], (float)VA1->estimate()[2],
                           (float)VG1->estimate()[0], (float)VG1->estimate()[1], (float)VG1->estimate()[2]);
...
//        TODO Maybe Reintegrate inertial measurments when difference between linearization point and current estimate is too big
        const auto *VP1 = dynamic_cast<const VertexPose *>(_vertices[0]);
        const auto *VV1 = dynamic_cast<const VertexVelocity *>(_vertices[1]);
        const auto *VG = dynamic_cast<const VertexGyroBias *>(_vertices[2]);
        const auto *VA = dynamic_cast<const VertexAccBias *>(_vertices[3]);
        const auto *VP2 = dynamic_cast<const VertexPose *>(_vertices[4]);
        const auto *VV2 = dynamic_cast<const VertexVelocity *>(_vertices[5]);
        const auto *VGDir = dynamic_cast<const VertexGDir *>(_vertices[6]);
        const auto *VS = dynamic_cast<const VertexScale *>(_vertices[7]);
        const IMU::Bias b((float)VA->estimate()[0], (float)VA->estimate()[1], (float)VA->estimate()[2],
                          (float)VG->estimate()[0], (float)VG->estimate()[1], (float)VG->estimate()[2]);
 ...
 */
    void EdgePriorPoseImu::computeError() {
        ...
        const auto *VV = dynamic_cast<const VertexVelocity *>(_vertices[1]);
        const auto *VG = dynamic_cast<const VertexGyroBias *>(_vertices[2]);
        const auto *VA = dynamic_cast<const VertexAccBias *>(_vertices[3]);
...
//  SO3 FUNCTIONS
    ...
    [[maybe_unused]] Eigen::Matrix3d Skew(const Eigen::Vector3d &w) {
        Eigen::Matrix3d W;
