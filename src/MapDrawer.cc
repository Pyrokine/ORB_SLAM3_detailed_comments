...
namespace ORB_SLAM3 {
    ...
    bool MapDrawer::ParseViewerParamFile(cv::FileStorage &fSettings) {
        ...
        if (!node.empty()) {
            mKeyFrameLineWidth = float(node.real());
        } else {
        ...
        if (!node.empty()) {
            mGraphLineWidth = float(node.real());
        } else {
        ...
        if (!node.empty()) {
            mCameraLineWidth = float(node.real());
        } else {
...
//              Loops
                ...
                for (auto sLoopKF : sLoopKFs) {
                    if (sLoopKF->mnId < vpKF->mnId)
                        continue;
                    Eigen::Vector3f Owl = sLoopKF->GetCameraCenter();
                    glVertex3f(Ow(0), Ow(1), Ow(2));
