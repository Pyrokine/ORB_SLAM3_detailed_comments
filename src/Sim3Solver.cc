...
namespace ORB_SLAM3 {
    Sim3Solver::Sim3Solver(KeyFrame *pKF1, KeyFrame *pKF2, const vector<MapPoint *> &vpMatched12, const bool bFixScale, vector<KeyFrame *> vpKeyFrameMatchedMP) :
            mnIterations(0), mnBestInners(0), mbFixScale(bFixScale),
    ...
    void Sim3Solver::SetRansacParameters(double probability, int minInners, int maxIterations) {
        mRansacProb = probability;
        mRansacMinInners = minInners;
        mRansacMaxIts = maxIterations;

        N = int(mvpMapPoints1.size());  // number of correspondences

        mvbInnersIdx.resize(N);

//      Adjust Parameters according to number of correspondences
        float epsilon = float(mRansacMinInners) / float(N);
...
//      Set RANSAC iterations according to probability, epsilon, and max iterations
    ...
    Eigen::Matrix4f Sim3Solver::iterate(int nIterations, bool &bNoMore, vector<bool> &vbInners, int &nInners) {
        bNoMore = false;
        vbInners = vector<bool>(mN1, false);
        nInners = 0;
...
//          Get min set of points
            ...
            if (mnInnersIdx >= mnBestInners) {
                ...
                if (mnInnersIdx > mRansacMinInners) {
                    nInners = mnInnersIdx;
                    for (int i = 0; i < N; ++i)
                        if (mvbInnersIdx[i])
                            vbInners[mvnIndices1[i]] = true;
                    return mBestT12;
                }
            }
        }
    ...
    Eigen::Matrix4f Sim3Solver::iterate(int nIterations, bool &bNoMore, vector<bool> &vbInners, int &nInners, bool &bConverge) {
        bNoMore = false;
        bConverge = false;
        vbInners = vector<bool>(mN1, false);
        nInners = 0;
...
//          Get min set of points
    ...
    Eigen::Matrix4f Sim3Solver::find(vector<bool> &vbInners12, int &nInners) {
        bool bFlag;
        return iterate(mRansacMaxIts, bFlag, vbInners12, nInners);
    }
...
//      sRinv.copyTo(mT21i.rowRange(0,3).colRange(0,3));
    ...
    void Sim3Solver::CheckInners() {
        ...
        for (size_t i = 0; i < mvP1im1.size(); ++i) {
            ...
            if (err1 < float(mvnMaxError1[i]) && err2 < float(mvnMaxError2[i])) {
                mvbInnersIdx[i] = true;
                mnInnersIdx++;
            } else
