...
//                std::cerr << name << " optional parameter does not exist..." << std::endl;
            ...
            std::cerr << name << " parameter must be a float number, aborting..." << std::endl;
            exit(-1);
        } else {
            found = true;
...
//          Read intrinsic parameters
            ...
            if (sensor_ == System::STEREO || sensor_ == System::IMU_STEREO) {
                ...
                dynamic_cast<KannalaBrandt8 *>(calibration1_)->mvLappingArea = vOverlapping;
            }
        } else {
...
//              Update calibration
                ...
                if ((sensor_ == System::STEREO || sensor_ == System::IMU_STEREO) && cameraType_ != Rectified) {
                    ...
                    if (cameraType_ == KannalaBrandt) {
                        dynamic_cast<KannalaBrandt8 *>(calibration1_)->mvLappingArea[0] *= int(scaleColFactor);
                        dynamic_cast<KannalaBrandt8 *>(calibration1_)->mvLappingArea[1] *= int(scaleColFactor);

                        dynamic_cast<KannalaBrandt8 *>(calibration2_)->mvLappingArea[0] *= int(scaleColFactor);
                        dynamic_cast<KannalaBrandt8 *>(calibration2_)->mvLappingArea[1] *= int(scaleColFactor);
                    }
                }
            }
        }
...
//      Update calibration
        calibration1_->setParameter(float(P1.at<double>(0, 0)), 0);
        calibration1_->setParameter(float(P1.at<double>(1, 1)), 1);
        calibration1_->setParameter(float(P1.at<double>(0, 2)), 2);
        calibration1_->setParameter(float(P1.at<double>(1, 2)), 3);

//      Update bf
        bf_ = float(b_ * P1.at<double>(0, 0));
...
//      Update relative pose between camera 1 and IMU if necessary
    ...
    ostream &operator<<(std::ostream &output, const Settings &settings) {
        ...
        for (size_t i = 0; i < settings.originalCalib1_->size(); ++i) {
            output << " " << settings.originalCalib1_->getParameter(int(i));
        }
        output << " ]" << endl;
        ...
        if (settings.sensor_ == System::STEREO || settings.sensor_ == System::IMU_STEREO) {
            if (settings.cameraType_ != Settings::Rectified) {
                ...
                for (size_t i = 0; i < settings.originalCalib2_->size(); ++i) {
                    output << " " << settings.originalCalib2_->getParameter(int(i));
                }
                output << " ]" << endl;
        ...
        if (settings.bNeedToRectify_) {
            ...
            for (size_t i = 0; i < settings.calibration1_->size(); ++i) {
                output << " " << settings.calibration1_->getParameter(int(i));
            }
            output << " ]" << endl;
            ...
            if ((settings.sensor_ == System::STEREO || settings.sensor_ == System::IMU_STEREO) &&
                ...
                for (size_t i = 0; i < settings.calibration2_->size(); ++i) {
                    output << " " << settings.calibration2_->getParameter(int(i));
                }
                output << " ]" << endl;
            }
        }
