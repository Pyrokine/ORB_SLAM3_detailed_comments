...
//  用于Tracking::SearchLocalPoints中匹配更多地图点
    int ORBMatcher::SearchByProjection(Frame &F, const vector<MapPoint *> &vpMapPoints, const float th, const bool bFarPoints, const float thFarPoints) const {
...
//                  Get best and second matches with near key points
                    for (unsigned long idx : vIndices) {
                        ...
                        const cv::Mat &d = F.mDescriptors.row(int(idx) + F.Nleft);
...
//      当视角相差小于3.6°，对应cos(3.6°)=0.998，搜索范围是2.5，否则是4
     ...
     * @param[in] pKF               关键帧
     * @param[in] F                 当前普通帧
     * @param[in] vpMapPointMatches F中地图点对应的匹配，nullptr表示未匹配
     * @return int                  成功匹配的数量
     */
...
//                  取出KF中该特征对应的地图点
                    ...
                    if (pMP->isBad())
                        continue;
//                  取出关键帧KF中该特征对应的描述子
                    const cv::Mat &dKF = pKF->mDescriptors.row(int(realIdxKF));
...
//                          如果地图点存在，说明这个点已经被匹配过了，不再匹配，加快速度
                            if (vpMapPointMatches[realIdxF])
                                continue;
//                          取出普通帧F中该特征对应的描述子
                            const cv::Mat &dF = F.mDescriptors.row(int(realIdxF));
//                          计算描述子的距离
...
//                              所有的特征点的角度变化应该是一致的，通过直方图统计得到最准确的角度变化值
                                ...
                                if (rot < 0.0)
                                    rot += 360.0f;
                                auto bin = int(round(rot * factor));  // 将rot分配到bin组, 四舍五入, 其实就是离散到对应的直方图组中
                                if (bin == HISTO_LENGTH)
                                    bin = 0;
...
//      使用set类型，记录前面已经成功的匹配关系，避免重复匹配。并去除其中无效匹配关系（nullptr）
...
//          Search in a radius
//          根据尺度确定搜索半径
            const auto radius = float(th) * pKF->mvScaleFactors[nPredictedLevel];
...
//          该MapPoint与bestIdx对应的特征点匹配成功
            if (float(bestDist) <= TH_LOW * ratioHamming) {
...
//          Search in a radius
            const float radius = float(th) * pKF->mvScaleFactors[nPredictedLevel];
...
//      Step 6 检查旋转直方图分布，剔除差异较大的匹配
    ...
    int ORBMatcher::SearchForTriangulation(KeyFrame *pKF1, KeyFrame *pKF2, vector<pair<size_t, size_t> > &vMatchedPairs, const bool bOnlyStereo, const bool bCoarse) const {
        const DBoW2::FeatureVector &vFeatVec1 = pKF1->mFeatVec;
        const DBoW2::FeatureVector &vFeatVec2 = pKF2->mFeatVec;

//      Compute epi-pole in second image
...
//                  由于寻找的是未匹配的特征点，所以pMP1应该为nullptr
                    if (pMP1) {
                        continue;
                    }
        ...
        if (mbCheckOrientation) {
            ...
            for (int i = 0; i < HISTO_LENGTH; ++i) {
                if (i == ind1 || i == ind2 || i == ind3)
                    continue;
                for (int j : rotHist[i]) {
                    vbMatched2[vMatches12[j]] = false;  // !清除匹配关系。原作者漏掉！
                    vMatches12[j] = -1;
                    nMatches--;
                }
            }
...
//      Step 4 存储匹配关系，下标是关键帧1的特征点id，存储的是关键帧2的特征点id
    ...
    int ORBMatcher::Fuse(KeyFrame *pKF, const vector<MapPoint *> &vpMapPoints, const float th, const bool bRight) {
        ...
        const auto nMPs = int(vpMapPoints.size());

//      For debuting
        int cnt_notMP = 0, cnt_bad = 0, cnt_isinKF = 0, cnt_negDepth = 0, cnt_notInIm = 0, cnt_dist = 0, cnt_normal = 0, cnt_notIdx = 0, cnt_thCheck = 0;
//      遍历所有的待投影地图点
...
//      与当前帧闭环匹配上的关键帧及其共视关键帧组成的地图点
        const auto nPoints = int(vpPoints.size());
...
//      取出关键帧中的地图点
        const vector<MapPoint *> vpMapPoints1 = pKF1->GetMapPointMatches();
        const auto N1 = int(vpMapPoints1.size());

        const vector<MapPoint *> vpMapPoints2 = pKF2->GetMapPointMatches();
        const auto N2 = int(vpMapPoints2.size());
...
//                  投影到当前帧中
                    ...
                    if (uv(0) < ORB_SLAM3::Frame::mnMinX || uv(0) > ORB_SLAM3::Frame::mnMaxX)
                        continue;
                    if (uv(1) < ORB_SLAM3::Frame::mnMinY || uv(1) > ORB_SLAM3::Frame::mnMaxY)
                        continue;
//                  认为投影前后地图点的尺度信息不变
...
//                      Search in a window. Size depends on scale
                        ...
                        for (unsigned long i2 : vIndices2) {
                            ...
                            const cv::Mat &d = CurrentFrame.mDescriptors.row(int(i2) + CurrentFrame.Nleft);
