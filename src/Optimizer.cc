    ...
    void Optimizer::InertialOptimization(Map *pMap, Eigen::Matrix3d &Rwg, double &scale) {
        ...
        for (size_t i = 0; i < vpKFs.size(); i++) {
            ...
            if (pKFi->mPrevKF && pKFi->mnId <= maxKFid) {
                ...
                if (!VP1 || !VV1 || !VG || !VA || !VP2 || !VV2 || !VGDir || !VS) {
                    Verbose::PrintMess("Error" + to_string(VP1->id()) + ", " + to_string(VV1->id()) + ", " + to_string(VG->id()) + ", " + to_string(VA->id()) + ", " + to_string(VP2->id()) + ", " + to_string(VV2->id()) + ", " + to_string(VGDir->id()) + ", " + to_string(VS->id()),
...
/**
 * @brief 没用到
 * 跟同名函数不同的地方在于输入是kfs，而不是地图，优化的目标有：
 * 速度，偏置
 * @param vpKFs 要更新的帧
 * @param bg 陀螺仪偏置（输出cout用）
 * @param ba 加速度计偏置（输出cout用）
