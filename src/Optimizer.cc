...
//      自由度为3的卡方分布，显著性水平为0.05，对应的临界阈值7.815
        const float deltaStereo = sqrt(7.815f);
...
//                          双目下没有加入相机
                            e->fx = double(ORB_SLAM3::Frame::fx);
                            e->fy = double(ORB_SLAM3::Frame::fy);
                            e->cx = double(ORB_SLAM3::Frame::cx);
                            e->cy = double(ORB_SLAM3::Frame::cy);
                            e->bf = pFrame->mbf;
                            e->Xw = pMP->GetWorldPos().cast<double>();
...
//      We perform 4 optimizations, after each optimization we classify observation as inner/exterior
//      At the next optimization, exteriors are not included, but at the end they can be classified as inners again.
//      Step 4：开始优化，总共优化四次，每次优化迭代十次,每次优化后，将观测分为 exterior 和 inner ， exterior 不参与下次优化
//      由于每次优化后是对所有的观测进行 exterior 和 inner 判别，因此之前被判别为 exterior 有可能变成 inner ，反之亦然
...
//              如果这条误差边是来自于exterior，也就是上一次优化去掉的
//              重新计算，因为上一次外点这一次可能成为内点
                if (pFrame->mvbExterior[idx]) {
                    e->computeError();
                }
...
//              就是error*\Omega*error,表征了这个点的误差大小(考虑置信度以后)
                ...
                if (chi2 > chi2Mono[it]) {
                    pFrame->mvbExterior[idx] = true;
                    e->setLevel(1);  // 设置为exterior , level 1 对应为外点,上面的过程中我们设置其为不优化
                    nBad++;
                } else {
                    pFrame->mvbExterior[idx] = false;
                    e->setLevel(0);  // 设置为inner, level 0 对应为内点,上面的过程中我们就是要优化这些关系
                }

                if (it == 2)
                    e->setRobustKernel(nullptr);  // 除了前两次优化需要RobustKernel以外, 其余的优化都不需要 -- 因为重投影的误差已经有明显的下降了
            }
//          对于相机2
...
//      Recover optimized pose and return number of inners
//      Step 5 得到优化后的当前帧的位姿
...
//          内点总数=单目内点数+双目内点数
            nInner = nInnerMono + nInnerStereo;
//          坏点数=单目坏点数+双目坏点数
...
//      给当前帧设置优化后的bg，ba
        pFrame->mImuBias = IMU::Bias((float)b[3], (float)b[4], (float)b[5], (float)b[0], (float)b[1], (float)b[2]);
...
//      We perform 4 optimizations, after each optimization we classify observation as inner/exterior
//      At the next optimization, exteriors are not included, but at the end they can be classified as inner again.
...
//      ?多此一举？优化过程中 nInner 这个值已经计算过了，nInnerMono 和 nInnerStereo 在后续代码中一直保持不变
        nInner = nInnerMono + nInnerStereo;
...
//      所以这个个数大约是点数×帧数，实际肯定比这个要少
        const auto nExpectedSize = int((lLocalKeyFrames.size() + lFixedCameras.size()) * lLocalMapPoints.size());
...
//      添加顶点：MapPoint
        for (auto pMP : lLocalMapPoints) {
            ...
            auto id = int(pMP->mnId + maxKFid + 1);
            vPoint->setId(id);
...
//      Keyframes
        ...
        for (auto pKFi : lLocalKeyFrames) {
            auto *vSE3 = dynamic_cast<g2o::VertexSE3Expmap *>(optimizer.vertex(int(pKFi->mnId)));
...
//      Points
        for (auto pMP : lLocalMapPoints) {
            auto *vPoint = dynamic_cast<g2o::VertexSBAPointXYZ *>(optimizer.vertex(int(pMP->mnId + maxKFid + 1)));
...
//      预计待优化的关键帧数，min函数是为了控制优化关键帧的数量
        ...
        int N = int(vpOptimizableKFs.size());
...
//      10. 建立惯性边，没有imu跳过
        for (int i = 0; i < N; ++i) {
            ...
            if (pKFi->bImu && pKFi->mPrevKF->bImu && pKFi->mpImuPreintegrated) {
                ...
                g2o::HyperGraph::Vertex *VP1 = optimizer.vertex(int(pKFi->mPrevKF->mnId));
                g2o::HyperGraph::Vertex *VV1 = optimizer.vertex(int(maxKFid + 3 * (pKFi->mPrevKF->mnId) + 1));
                g2o::HyperGraph::Vertex *VG1 = optimizer.vertex(int(maxKFid + 3 * (pKFi->mPrevKF->mnId) + 2));
                g2o::HyperGraph::Vertex *VA1 = optimizer.vertex(int(maxKFid + 3 * (pKFi->mPrevKF->mnId) + 3));
                g2o::HyperGraph::Vertex *VP2 = optimizer.vertex(int(pKFi->mnId));
                g2o::HyperGraph::Vertex *VV2 = optimizer.vertex(int(maxKFid + 3 * (pKFi->mnId) + 1));
                g2o::HyperGraph::Vertex *VG2 = optimizer.vertex(int(maxKFid + 3 * (pKFi->mnId) + 2));
                g2o::HyperGraph::Vertex *VA2 = optimizer.vertex(int(maxKFid + 3 * (pKFi->mnId) + 3));
...
//                  last optimizable keyframe inside the local window and the first fixed keyframe out. The
//                  information matrix for this measurement is also down-weighted. This is done to avoid accumulating
...
//      Set MapPoint vertices
        const auto nExpectedSize = int((N + lFixedKeyFrames.size()) * lLocalMapPoints.size());
...
//      12. 开始优化
        ...
        auto err = float(optimizer.activeRobustChi2());
        optimizer.optimize(opt_it);  // Originally to 2
        auto err_end = float(optimizer.activeRobustChi2());
        if (pbStopFlag)
            optimizer.setForceStopFlag(pbStopFlag);
...
//      Get Map Mutex and erase exterior
        unique_lock<mutex> lock(pMap->mMutexMapUpdate);
...
//      Local temporal Keyframes
        ...
        for (int i = 0; i < N; ++i) {
            ...
            if (pKFi->bImu) {
                auto *VV = dynamic_cast<VertexVelocity *>(optimizer.vertex(int(maxKFid + 3 * (pKFi->mnId) + 1)));
                pKFi->SetVelocity(VV->estimate().cast<float>());
                auto *VG = dynamic_cast<VertexGyroBias *>(optimizer.vertex(int(maxKFid + 3 * (pKFi->mnId) + 2)));
                auto *VA = dynamic_cast<VertexAccBias *>(optimizer.vertex(int(maxKFid + 3 * (pKFi->mnId) + 3)));
                Vector6d b;
                b << VG->estimate(), VA->estimate();
                pKFi->SetNewBias(IMU::Bias((float)b[3], (float)b[4], (float)b[5], (float)b[0], (float)b[1], (float)b[2]));
            }
        }
...
//      Points
        for (auto pMP : lLocalMapPoints) {
            auto *vPoint = dynamic_cast<g2o::VertexSBAPointXYZ *>(optimizer.vertex(int(pMP->mnId + iniMPid + 1)));
            pMP->SetWorldPos(vPoint->estimate().cast<float>());
            pMP->UpdateNormalAndDepth();
        }
...
//      卡方分布 95% 以上可信度的时候的阈值
        const float thHuber2D = sqrt(5.99f);  // 自由度为2
        const float thHuber3D = sqrt(7.815f);  // 自由度为3
...
//              滤出不合法的关键帧
                if (pKF->isBad() || pKF->mnId > maxKFid)
                    continue;
                if (optimizer.vertex(id) == nullptr || optimizer.vertex(int(pKF->mnId)) == nullptr)
                    continue;
                nEdges++;
...
//      遍历所有的关键帧
        for (auto pKF : vpKFs) {
            if (pKF->isBad())
                continue;
//          获取到优化结果
            auto *vSE3 = dynamic_cast<g2o::VertexSE3Expmap *>(optimizer.vertex(int(pKF->mnId)));
...
//      priorA!=0.f 时 bInit为true，加入了偏置节点
        if (bInit) {
            ...
            VG->setId(4 * int(maxKFid) + 2);
            VG->setFixed(false);
            optimizer.addVertex(VG);
            auto *VA = new VertexAccBias(pIncKF);
            VA->setId(4 * int(maxKFid) + 3);
            VA->setFixed(false);
            optimizer.addVertex(VA);
        }
//      false
...
//                  根据不同输入配置相应的偏置节点
                    if (!bInit) {
                        VG1 = optimizer.vertex(int(maxKFid + 3 * (pKFi->mPrevKF->mnId) + 2));
                        VA1 = optimizer.vertex(int(maxKFid + 3 * (pKFi->mPrevKF->mnId) + 3));
...
//      只加入pIncKF帧的偏置，优化偏置到0
        if (bInit) {
            g2o::HyperGraph::Vertex *VG = optimizer.vertex(4 * int(maxKFid) + 2);
            g2o::HyperGraph::Vertex *VA = optimizer.vertex(4 * int(maxKFid) + 3);
...
//                  添加边
                    if (leftIndex != -1 && pKFi->mvuRight[get<0>(observation.second)] < 0)  // Monocular observation
                    {
                        ...
                        auto *VP = dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(int(pKFi->mnId)));
                        if (bAllFixed)
                            if (!VP->fixed())
                                bAllFixed = false;
...
//      Keyframes
        for (auto pKFi : vpKFs) {
            ...
            if (pKFi->bImu) {
                ...
                VertexAccBias *VA;
                if (!bInit) {
                    VG = dynamic_cast<VertexGyroBias *>(optimizer.vertex(int(maxKFid + 3 * (pKFi->mnId) + 2)));
                    VA = dynamic_cast<VertexAccBias *>(optimizer.vertex(int(maxKFid + 3 * (pKFi->mnId) + 3)));
                } else {
                    VG = dynamic_cast<VertexGyroBias *>(optimizer.vertex(int(4 * maxKFid + 2)));
                    VA = dynamic_cast<VertexAccBias *>(optimizer.vertex(int(4 * maxKFid + 3)));
                }

                Vector6d vb;
                vb << VG->estimate(), VA->estimate();
                IMU::Bias b((float)vb[3], (float)vb[4], (float)vb[5], (float)vb[0], (float)vb[1], (float)vb[2]);
                if (nLoopId == 0) {
                    pKFi->SetNewBias(b);
                } else {
                    pKFi->mBiasGBA = b;
...
//          跳过id大于当前地图最大id的关键帧
            if (pKFi->mnId > maxKFid)
                continue;
            auto *VP = new VertexPose(pKFi);  // 继承于public g2o::BaseVertex<6, ImuCamPose>
            VP->setId(int(pKFi->mnId));
            VP->setFixed(true);
            optimizer.addVertex(VP);

            auto *VV = new VertexVelocity(pKFi);  // 继承于public g2o::BaseVertex<3, Eigen::Vector3d>
...
//              将pKFi偏置设定为上一关键帧的偏置
                pKFi->mpImuPreintegrated->SetNewBias(pKFi->mPrevKF->GetImuBias());
                g2o::HyperGraph::Vertex *VP1 = optimizer.vertex(int(pKFi->mPrevKF->mnId));
                g2o::HyperGraph::Vertex *VV1 = optimizer.vertex(int(maxKFid + (pKFi->mPrevKF->mnId) + 1));
                g2o::HyperGraph::Vertex *VP2 = optimizer.vertex(int(pKFi->mnId));
                g2o::HyperGraph::Vertex *VV2 = optimizer.vertex(int(maxKFid + (pKFi->mnId) + 1));
                g2o::HyperGraph::Vertex *VG = optimizer.vertex(int(maxKFid * 2 + 2));
                g2o::HyperGraph::Vertex *VA = optimizer.vertex(int(maxKFid * 2 + 3));
                g2o::HyperGraph::Vertex *VGDir = optimizer.vertex(int(maxKFid * 2 + 4));
                g2o::HyperGraph::Vertex *VS = optimizer.vertex(int(maxKFid * 2 + 5));
...
//      Keyframes velocities and biases
        const int N = int(vpKFs.size());
        for (size_t i = 0; i < N; ++i) {
            KeyFrame *pKFi = vpKFs[i];
            if (pKFi->mnId > maxKFid)
                continue;

            auto *VV = dynamic_cast<VertexVelocity *>(optimizer.vertex(int(maxKFid + (pKFi->mnId) + 1)));
...
//        4. 添加边
        ...
        for (auto pKFi : vpKFs) {
            if (pKFi->mPrevKF && pKFi->mnId <= maxKFid) {
                ...
                g2o::HyperGraph::Vertex *VP1 = optimizer.vertex(int(pKFi->mPrevKF->mnId));
                g2o::HyperGraph::Vertex *VV1 = optimizer.vertex(int((maxKFid + 1) + pKFi->mPrevKF->mnId));
                g2o::HyperGraph::Vertex *VP2 = optimizer.vertex(int(pKFi->mnId));
                g2o::HyperGraph::Vertex *VV2 = optimizer.vertex(int((maxKFid + 1) + pKFi->mnId));
                g2o::HyperGraph::Vertex *VG = optimizer.vertex(int(2 * (maxKFid + 1) + pKFi->mPrevKF->mnId));
                g2o::HyperGraph::Vertex *VA = optimizer.vertex(int(3 * (maxKFid + 1) + pKFi->mPrevKF->mnId));
                g2o::HyperGraph::Vertex *VGDir = optimizer.vertex(int(4 * (maxKFid + 1)));
                g2o::HyperGraph::Vertex *VS = optimizer.vertex(int(4 * (maxKFid + 1) + 1));
                if (!VP1 || !VV1 || !VG || !VA || !VP2 || !VV2 || !VGDir || !VS) {
 ...
 * @param pKF1         当前关键帧
 * @param pKF2         候选关键帧
 * @param vpMatches1   当前关键帧与地图匹配上的MP，中间会有nullptr，与当前关键帧的特征点一一对应
 * @param g2oS12       相似变换矩阵
 * @param th2          误差上限的平方
...
//          如果是imu的话还会找前一帧做优化
            if (pKF->bImu && pKF->mPrevKF) {
                ...
                ep->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(int(pKF->mPrevKF->mnId))));
...
//      Edge used in pose-graph has still 6Dof, even if updates of camera poses are just in 4DoF
...
//          Use non-corrected poses for pose-graph edges
            auto iti = NonCorrectedSim3.find(pKF);
...
//                  同样，认为相对位姿比较准
                    ...
                    e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(nIDi)));
                    e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(int(pLKF->mnId))));
                    e->information() = matLambda;
                    optimizer.addEdge(e);
                }
            }
...
//              在之前没有添加过
                if (pKFn && pKFn != pParentKF && pKFn != prevKF && pKFn != pKF->mNextKF && !pKF->hasChild(pKFn) && !sLoopEdges.count(pKFn)) {
                    if (!pKFn->isBad() && pKFn->mnId < pKF->mnId) {
                        ...
                        e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(nIDi)));
                        e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(int(pKFn->mnId))));
                        e->information() = matLambda;
                        optimizer.addEdge(e);
                    }
                }
...
//      6. 设定优化后的位姿
        for (auto pKFi : vpKFs) {
            ...
            auto *Vi = dynamic_cast<VertexPose4DoF *>(optimizer.vertex(nIDi));
 ...
 */
    void Optimizer::LocalBundleAdjustment(
            KeyFrame *pMainKF, vector<KeyFrame *> vpAdjustKF, const vector<KeyFrame *>& vpFixedKF, bool *pbStopFlag) {
...
//      准备存放边的vector
        const auto nExpectedSize = int((vpAdjustKF.size() + vpFixedKF.size()) * vpMPs.size());
...
//      更新点的坐标
        for (MapPoint *pMPi: vpMPs) {
            ...
            auto *vPoint = dynamic_cast<g2o::VertexSBAPointXYZ *>(optimizer.vertex(int(pMPi->mnId + maxKFid + 1)));
...
//      1. 优化器构建
        ...
        const int minFeat = 100;  // pKFi->GetCovisiblesByWeight(minFeat);  essential graph 阈值就是共视大于100
 ...
 */
    void Optimizer::MergeInertialBA(KeyFrame *pCurrKF, KeyFrame *pMergeKF, const bool *pbStopFlag, Map *pMap, LoopClosing::KeyFrameAndPose &corrPoses) {
        const int Nd = 6;
...
//      2.5 数量不够时，添加最后一个的下一帧。继续添加直到达到2Nd个可优化关键帧,或没有新的可以添加了
        ...
        auto N = int(vpOptimizableKFs.size());
...
//          遍历每个关键帧所有的地图点
            for (auto pMP : vpMPs) {
//                Using mnBALocalForKF we avoid redundancy here, one MP can not be added several times to lLocalMapPoints
                if (pMP)
                    if (!pMP->isBad())
//                      用这个变量记录是否重复添加
...
//      5.2 优化位姿，速度与偏置
        auto Ncov = int(vpOptimizableCovKFs.size());
...
//      6.2 设置所有地图的顶点
//      设置地图点顶点
        const auto nExpectedSize = int((N + Ncov + lFixedKeyFrames.size()) * lLocalMapPoints.size());
...
//          添加重投影边
            for (const auto & observation : observations) {
                ...
                if (optimizer.vertex(int(id)) == nullptr || optimizer.vertex(int(pKFi->mnId)) == nullptr)
                    continue;
