...
//  For stereo fisheye matching
    ...
    Frame::Frame() :
            mpcpi(nullptr), mpImuPreintegrated(nullptr), mpPrevFrame(nullptr), mpImuPreintegratedFrame(nullptr), mpReferenceKF(static_cast<KeyFrame *>(nullptr)),
...
//  Copy Constructor
    Frame::Frame(const Frame &frame)
            : mpcpi(frame.mpcpi), mpORBvocabulary(frame.mpORBvocabulary), mpORBextractorLeft(frame.mpORBextractorLeft), mpORBextractorRight(frame.mpORBextractorRight),
              mTimeStamp(frame.mTimeStamp), mK(frame.mK.clone()), mK_(Converter::toMatrix3f(frame.mK)), mDistCoef(frame.mDistCoef.clone()), mbf(frame.mbf), mb(frame.mb),
              mThDepth(frame.mThDepth), N(frame.N), mvKeys(frame.mvKeys), mvKeysRight(frame.mvKeysRight), mvKeysUn(frame.mvKeysUn), mvuRight(frame.mvuRight), mvDepth(frame.mvDepth),
              ...
              mvpMapPoints(frame.mvpMapPoints), mvbExterior(frame.mvbExterior), mImuCalib(frame.mImuCalib), mnCloseMPs(frame.mnCloseMPs), mpImuPreintegrated(frame.mpImuPreintegrated),
              mpImuPreintegratedFrame(frame.mpImuPreintegratedFrame), mImuBias(frame.mImuBias), mnId(frame.mnId), mpReferenceKF(frame.mpReferenceKF), mnScaleLevels(frame.mnScaleLevels),
              ...
              mNameFile(frame.mNameFile), mnDataset(frame.mnDataset), mvLevelSigma2(frame.mvLevelSigma2), mvInvLevelSigma2(frame.mvInvLevelSigma2), mpPrevFrame(frame.mpPrevFrame),
              mpLastKeyFrame(frame.mpLastKeyFrame), mbIsSet(frame.mbIsSet), mbImuPreintegrated(frame.mbImuPreintegrated), mpMutexImu(frame.mpMutexImu), mpCamera(frame.mpCamera),
              mpCamera2(frame.mpCamera2), Nleft(frame.Nleft), Nright(frame.Nright), monoLeft(frame.monoLeft), monoRight(frame.monoRight), mvLeftToRightMatch(frame.mvLeftToRightMatch),
              mvRightToLeftMatch(frame.mvRightToLeftMatch), mvStereo3Dpoints(frame.mvStereo3Dpoints), mTlr(frame.mTlr), mRlr(frame.mRlr), mtlr(frame.mtlr), mTrl(frame.mTrl),
...
//  立体匹配模式下的双目
    Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp, ORBExtractor *extractorLeft, ORBExtractor *extractorRight, ORBVocabulary *voc,
            ...
            : mpcpi(nullptr), mpORBvocabulary(voc), mpORBextractorLeft(extractorLeft), mpORBextractorRight(extractorRight), mTimeStamp(timeStamp), mK(K.clone()),
              mK_(Converter::toMatrix3f(K)), mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth), mImuCalib(ImuCalib), mpImuPreintegrated(nullptr),
              mpPrevFrame(pPrevF), mpImuPreintegratedFrame(nullptr), mpReferenceKF(static_cast<KeyFrame *>(nullptr)), mbIsSet(false), mbImuPreintegrated(false),
...
//      初始化本帧的地图点
        ...
        mvbExterior = vector<bool>(N, false);
...
//  单目模式
    Frame::Frame(const cv::Mat &imGray, const double &timeStamp, ORBExtractor *extractor, ORBVocabulary *voc, GeometricCamera *pCamera, cv::Mat &distCoef, const float &bf,
                 const float &thDepth, Frame *pPrevF, const IMU::Calib &ImuCalib)
            : mpcpi(nullptr), mpORBvocabulary(voc), mpORBextractorLeft(extractor), mpORBextractorRight(static_cast<ORBExtractor *>(nullptr)), mTimeStamp(timeStamp),
              mK(dynamic_cast<Pinhole *>(pCamera)->toK()), mK_(dynamic_cast<Pinhole *>(pCamera)->toK_()), mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth),
              mImuCalib(ImuCalib), mpImuPreintegrated(nullptr), mpPrevFrame(pPrevF), mpImuPreintegratedFrame(nullptr), mpReferenceKF(static_cast<KeyFrame *>(nullptr)),
              mbIsSet(false), mbImuPreintegrated(false), mpCamera(pCamera), mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false) {
//      Frame ID
//      Step 1 帧的ID 自增
        mnId = nNextId++;
...
//      Step 1  给存储特征点的网格数组 Frame::mGrid 预分配空间
        ...
        auto nReserve = int(0.5 * double(N) / (nCells));
 ...
 * @brief 获得imu的位姿
 */
    [[maybe_unused]] Sophus::SE3<float> Frame::GetImuPose() {
 ...
 * @brief 获得左右目的相对旋转
 */
    [[maybe_unused]] Eigen::Matrix3f Frame::GetRelativePoseTlr_rotation() {
 ...
 * @brief 获得左右目的相对平移
 */
    [[maybe_unused]] Eigen::Vector3f Frame::GetRelativePoseTlr_translation() {
 ...
 * @brief 暂没用到
 */
    [[maybe_unused]] bool Frame::ProjectPointDistort(MapPoint *pMP, cv::Point2f &kp, float &u, float &v) {
 ...
 * @brief 暂没用到
 */
    [[maybe_unused]] Eigen::Vector3f Frame::inRefCoordinates(const Eigen::Vector3f &pCw) {
...
//          获取左图特征点il所在行，以及在右图对应行中可能的匹配点
            const vector<size_t> &vCandidates = vRowIndices[int(vL)];
...
//              提取左图中，以特征点(scaleduL,scaledvL)为中心, 半径为w的图像快patch
                cv::Mat IL = mpORBextractorLeft->mvImagePyramid[kpL.octave].rowRange(int(scaledvL) - w, int(scaledvL) + w + 1)
                        .colRange(int(scaleduL) - w, int(scaleduL) + w + 1);
...
//              判断搜索是否越界
                if (iniu < 0 || endu >= float(mpORBextractorRight->mvImagePyramid[kpL.octave].cols))
                    continue;

//              在搜索范围内从左到右滑动，并计算图像块相似度
                for (int incR = -L; incR <= +L; ++incR) {
//                  提取左图中，以特征点(scaleduL,scaledvL)为中心, 半径为w的图像快patch
                    cv::Mat IR = mpORBextractorRight->mvImagePyramid[kpL.octave].rowRange(int(scaledvL) - w, int(scaledvL) + w + 1).
                            colRange(int(scaleduR0) + incR - w, int(scaleduR0) + incR + w + 1);

//                  sad 计算
                    auto dist = float(cv::norm(IL, IR, cv::NORM_L1));
//                  统计最小sad和偏移量
                    if (dist < float(bestDist)) {
                        bestDist = int(dist);
                        bestincR = incR;
                    }
...
//          NOTE 从这里看对深度图像进行去畸变处理是没有必要的,我们依旧可以直接通过未矫正的特征点的坐标来直接拿到深度数据
            const float d = imDepth.at<float>(int(v), int(u));
 ...
 */
    bool Frame::UnprojectedStereo(const int &i, Eigen::Vector3f &x3D) {
 ...
 */
    Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp, ORBExtractor *extractorLeft, ORBExtractor *extractorRight, ORBVocabulary *voc,
                 cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera *pCamera, GeometricCamera *pCamera2, Sophus::SE3f &Tlr, Frame *pPrevF,
                 const IMU::Calib &ImuCalib)
            : mpcpi(nullptr), mpORBvocabulary(voc), mpORBextractorLeft(extractorLeft), mpORBextractorRight(extractorRight), mTimeStamp(timeStamp), mK(K.clone()),
              mK_(Converter::toMatrix3f(K)), mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth), mImuCalib(ImuCalib), mpImuPreintegrated(nullptr),
              mpPrevFrame(pPrevF), mpImuPreintegratedFrame(nullptr), mpReferenceKF(static_cast<KeyFrame *>(nullptr)), mbImuPreintegrated(false), mpCamera(pCamera),
              mpCamera2(pCamera2), mbHasPose(false), mbHasVelocity(false) {
...
//      左图中提取的特征点数目
        Nleft = int(mvKeys.size());
//      右图中提取的特征点数目
        Nright = int(mvKeysRight.size());
//      特征点总数
        N = Nleft + Nright;
...
//      1. 分别取出特征点
//      Speed it up by matching key points in the lapping area
...
//              三角化
                float depth = dynamic_cast<KannalaBrandt8 *>(mpCamera)->TriangulateMatches(
                                mpCamera2, mvKeys[match[0].queryIdx + monoLeft], mvKeysRight[match[0].trainIdx + monoRight],
 ...
 */
    Eigen::Vector3f Frame::UnprojectedStereoFishEye(const int &i) {
