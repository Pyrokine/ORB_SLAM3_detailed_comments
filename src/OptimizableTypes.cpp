 ...
 */
    void EdgeSE3ProjectXYZOnlyPose::linearizeOplus() {
        auto *vi = dynamic_cast<g2o::VertexSE3Expmap *>(_vertices[0]);
        ...
        _jacobianOplusXi = -pCamera->projectedJac(xyz_trans) * SE3deriv;
    }
...
//      让Pr 对 ξlw 求雅克比
//      相当于Rrl*(Pl 对 ξlw的雅克比)
        _jacobianOplusXi = -pCamera->projectedJac(X_r) * mTrl.rotation().toRotationMatrix() * SE3deriv;
    }
 ...
 */
    void EdgeSE3ProjectXYZ::linearizeOplus() {
        auto *vj = dynamic_cast<g2o::VertexSE3Expmap *>(_vertices[1]);
        g2o::SE3Quat T(vj->estimate());
        auto *vi = dynamic_cast<g2o::VertexSBAPointXYZ *>(_vertices[0]);
        ...
        auto projectJac = -pCamera->projectedJac(xyz_trans);
 ...
 */
    void EdgeSE3ProjectXYZToBody::linearizeOplus() {
        ...
        _jacobianOplusXi = -pCamera->projectedJac(X_r) * T_rw.rotation().toRotationMatrix();
...
//      让Pr 对 ξlw 求雅克比
//      相当于Rrl*(Pl 对 ξlw的雅克比)
        _jacobianOplusXj = -pCamera->projectedJac(X_r) * mTrl.rotation().toRotationMatrix() * SE3deriv;
    }
    ...
    bool VertexSim3Expmap::write(std::ostream &os) const {
        ...
        for (size_t i = 0; i < pCamera1->size(); i++) {
            os << pCamera1->getParameter(int(i)) << " ";
        }

        for (size_t i = 0; i < pCamera2->size(); i++) {
            os << pCamera2->getParameter(int(i)) << " ";
        }
