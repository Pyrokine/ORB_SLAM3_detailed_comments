...
//            mpViewer->AddMapToCreateThumbnail(mpCurrentMap);
        ...
        mpCurrentMap = new Map(int(mnLastInitKFidMap));  // 新建地图
    ...
    [[maybe_unused]] void Atlas::SetViewer(Viewer *pViewer) {
        mpViewer = pViewer;
        mHasViewer = true;
    }
...
//      遍历地图中现有的相机看看跟输入的相机一不一样，不一样的话则向mvpCameras添加
    ...
    [[maybe_unused]] std::vector<MapPoint *> Atlas::GetReferenceMapPoints() {
        unique_lock<mutex> lock(mMutexAtlas);
...
//          如果地图为空，则跳过
            if (int(pMi->GetAllKeyFrames().size()) == 0) {
...
//      2. 加载各个地图
    ...
    [[maybe_unused]] KeyFrameDatabase *Atlas::GetKeyFrameDatabase() {
        return mpKeyFrameDB;
    }
 ...
 * @brief 以下函数暂未用到
 */
    [[maybe_unused]] ORBVocabulary *Atlas::GetORBVocabulary() {
        return mpORBVocabulary;
    }

    [[maybe_unused]] long unsigned int Atlas::GetNumLivedKF() {
        unique_lock<mutex> lock(mMutexAtlas);
    ...
    [[maybe_unused]] long unsigned int Atlas::GetNumLivedMP() {
        unique_lock<mutex> lock(mMutexAtlas);
        long unsigned int num = 0;
        for (Map *pMap_i: mspMaps) {
    ...
    [[maybe_unused]] map<long unsigned int, KeyFrame *> Atlas::GetAtlasKeyframes() {
