...
//          通过把候选帧局部窗口内的地图点向新进来的关键帧投影来验证回环检测结果,并优化Sim3位姿
            bool bCommonRegion = DetectAndRefineSim3FromLastKF(mpCurrentKF, mpLoopMatchedKF, gScw, numProjMatches, mvpLoopMPs, vpMatchedMPs);
//          如果找到共同区域(时序验证成功一次)
            if (bCommonRegion) {
//              标记时序检验成功一次
...
//              ! 这里的条件反了,不过对功能没什么影响,只是打印信息
                if (!mbLoopDetected) {
                    cout << "PR: Loop detected with Refine Sim3" << endl;
                }
            }
//          如果没找到共同区域(时序验证失败一次)
            else {
...
//          通过把候选帧局部窗口内的地图点向新进来的关键帧投影来验证回环检测结果,并优化Sim3位姿
            bool bCommonRegion = DetectAndRefineSim3FromLastKF(mpCurrentKF, mpMergeMatchedKF, gScw, numProjMatches, mvpMergeMPs, vpMatchedMPs);
//          如果找到共同区域(时序验证成功一次)
            if (bCommonRegion) {
//              标记时序检验成功一次
 ...
 */
    bool LoopClosing::DetectAndRefineSim3FromLastKF(KeyFrame *pCurrentKF, KeyFrame *pMatchedKF, g2o::Sim3 &gScw, int &nNumProjMatches,
...
//          Verbose::PrintMess("Sim3 refine: There are " + to_string(nNumProjMatches) + " initial matches ", Verbose::VERBOSITY_DEBUG);
...
//          如果是imu模式且未完成初始化,不锁定尺度
            if (mpTracker->mSensor == System::IMU_MONOCULAR && !pCurrentKF->GetMap()->GetInertialBA2())
                bFixedScale = false;
//          3.2 优化gScm，mp固定
            int numOptMatches = Optimizer::OptimizeSim3(mpCurrentKF, pMatchedKF, vpMatchedMPs, gScm, 10, bFixedScale, mHessian7x7, true);

//          Verbose::PrintMess("Sim3 refine: There are " + to_string(numOptMatches) + " matches after of the optimization ", Verbose::VERBOSITY_DEBUG);
//          若匹配的数量大于一定的数目
...
//      一些后面会使用的阀值
        ...
        int nBoWInners = 15;  // RANSAC最低的匹配点数
        int nSim3Inners = 20;  // sim3 最低内点数
...
//      bow中候选关键帧的数量
        int numCandidates = int(vpBowCand.size());
...
//              如果窗口中的帧是当前帧的共视帧则结束这个循环
                if (spConnectedKeyFrames.find(j) != spConnectedKeyFrames.end()) {
                    bAbortByNearKF = true;
                    break;
                }
            }
...
//          记录窗口中的地图点能在当前关键帧中找到的匹配的点(数量的上限是当前关键帧地图点的数量)
            std::vector<MapPoint *> vpMatchedPoints = std::vector<MapPoint *>(mpCurrentKF->GetMapPointMatches().size(), static_cast<MapPoint *>(nullptr));
//          记录上面的地图点分别对应窗口中的关键帧(数量的上限是当前关键帧地图点的数量)
            std::vector<KeyFrame *> vpKeyFrameMatchedMP = std::vector<KeyFrame *>(mpCurrentKF->GetMapPointMatches().size(), static_cast<KeyFrame *>(nullptr));
...
//              如果是单目带imu的模式且IMU初始化未完成第三阶段，则不固定scale
                if (mpTracker->mSensor == System::IMU_MONOCULAR && !mpCurrentKF->GetMap()->GetInertialBA2())
                    bFixedScale = false;

//              3.1 初始化sim3 solver
//              Sim3Solver 的接口与orb slam2略有不同, 因为现在是1-N的对应关系
                Sim3Solver solver = Sim3Solver(mpCurrentKF, pMostBoWMatchesKF, vpMatchedPoints, bFixedScale, vpKeyFrameMatchedMP);
//              Sim3Solver Ransac 置信度0.99，至少20个inners 最多300次迭
                solver.SetRansacParameters(0.99, nBoWInners, 300);//  at least 15 inners
...
//              3.2 迭代到收敛
                while (!bConverge && !bNoMore) {
                    mTcm = solver.iterate(20, bNoMore, vbInners, nInners, bConverge);
//                    Verbose::PrintMess("BoW guess: Solver achieve " + to_string(nInners) + " geometrical inners among " + to_string(nBoWInners) + " BoW matches", Verbose::VERBOSITY_DEBUG);
                }
...
//                    Verbose::PrintMess("BoW guess: Convergence with " + to_string(nInners) + " geometrical inners among " + to_string(nBoWInners) + " BoW matches", Verbose::VERBOSITY_DEBUG);
...
//                  记录最后searchByProjection的结果
                    ...
                    vpMatchedKF.resize(mpCurrentKF->GetMapPointMatches().size(), static_cast<KeyFrame *>(nullptr));
...
//                              以下为调试信息
                                ...
                                for (MapPoint *pMPi: vpMatchedMP) {
                                    ...
                                    int index = int(get<0>(indexes));
                                    if (index >= 0) {
                                        int coord_x = int(pKFi->mvKeysUn[index].pt.x);
                                        if (coord_x < min_x) {
                                            min_x = coord_x;
                                        }
                                        if (coord_x > max_x) {
                                        ...
                                        int coord_y = int(pKFi->mvKeysUn[index].pt.y);
                                        if (coord_y < min_y) {
                                            min_y = coord_y;
                                        }
                                        if (coord_y > max_y) {
...
//                  如果没有被重复添加且不是当前关键帧的共视关键帧
                    if (spCheckKFs.find(vpKFs[j]) == spCheckKFs.end() && spCurrentCovisibles.find(vpKFs[j]) == spCurrentCovisibles.end()) {
...
//      请求局部地图停止，防止在回环矫正时局部地图线程中InsertKeyFrame函数插入新的关键帧
        ...
        mpLocalMapper->EmptyQueue();  // Process keyframes in the queue

//      如果正在进行全局BA，丢弃它
//      If a Global Bundle Adjustment is running, abort it
        if (isRunningGBA()) {
            cout << "Stopping Global Bundle Adjustment...";
...
//          TODO Check this index increase
...
//                      如果当前帧没有该MapPoint，则直接添加
                        ...
                        pLoopMP->AddObservation(mpCurrentKF, int(i));
...
//      Loop closed. Release Local Mapping.
        ...
        mLastLoopKFid = mpCurrentKF->mnId;  // TODO old variable, it is not use in the new algorithm
    }
...
//      Later, the elements of the current map will be transformed to the new active map reference, in order to keep real time tracking
...
//          Optimize graph (and update the loop position for each element form the beginning to the end)
//          Step 8.2 本质图优化
            if (mpTracker->mSensor != System::MONOCULAR) {
...
//          mSold_new = gSw2w1 记录的是当前关键帧世界坐标系到融合关键帧世界坐标系的变换
            auto s_on = float(mSold_new.scale());
...
//      反正都要融合了，这里就拔苗助长完成IMU优化，回头直接全部放到融合地图里就好了
        const int numKFNew = int(pCurrentMap->KeyFramesInMap());

        if ((mpTracker->mSensor == System::IMU_MONOCULAR || mpTracker->mSensor == System::IMU_STEREO || mpTracker->mSensor == System::IMU_RGBD)
            && !pCurrentMap->GetInertialBA2()) {
//          Map is not completely initialized
            Eigen::Vector3d bg, ba;
            bg << 0., 0., 0.;
            ba << 0., 0., 0.;
//          优化当前地图中参数bg,ba
            Optimizer::InertialOptimization(pCurrentMap, bg, ba);
            IMU::Bias b((float)ba[0], (float)ba[1], (float)ba[2], (float)bg[0], (float)bg[1], (float)bg[2]);
            unique_lock<mutex> lock(mpAtlas->GetCurrentMap()->mMutexMapUpdate);
//          用优化得到的 bias 更新普通帧位姿
...
//      TODO Check: If new map is too small, we suppose that not information can be propagated from new to old map
        if (numKFNew < 10) {
            mpLocalMapper->Release();
            return;
        }
...
//          更新点
            const int nLP = int(vpMapPoints.size());
            for (int i = 0; i < nLP; ++i) {
 ...
 * @param vConnectedKFs 当前地图的当前关键帧及5个共视关键帧
 * @param vpMapPoints 待融合地图的融合帧及其5个共视关键帧对应的mp（1000个以内）（注意此时所有kf与mp全部移至当前地图，这里的待融合地图的说法只为区分，因为还没有融合）
 */
    void LoopClosing::SearchAndFuse(const vector<KeyFrame *> &vConnectedKFs, vector<MapPoint *> &vpMapPoints) {
        ORBMatcher matcher(0.8);

        int total_replaces = 0;

//        cout << "FUSE-POSE: Initially there are " << vpMapPoints.size() << " MPs" << endl;
//        cout << "FUSE-POSE: Initially there are " << vConnectedKFs.size() << " KFs" << endl;
...
//                cout << "LC: Update Map Mutex acquired" << endl;
...
//              通过树的方式更新未参与全局优化的关键帧，一个关键帧与其父节点的共视点数最多，所以选其作为参考帧
                while (!lpKFtoCheck.empty()) {
                    ...
                    const set<KeyFrame *> sChildren = pKF->GetChildren();
//                    cout << "---Updating KF " << pKF->mnId << " with " << sChildren.size() << " children" << endl;
