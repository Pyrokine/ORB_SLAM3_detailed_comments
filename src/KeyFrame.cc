...
namespace ORB_SLAM3 {
    ...
    KeyFrame::KeyFrame()
            : mnFrameId(0), mTimeStamp(0), mnGridCols(FRAME_GRID_COLS), mnGridRows(FRAME_GRID_ROWS),
              ...
              mnMaxY(0), mPrevKF(static_cast<KeyFrame *>(nullptr)), mNextKF(static_cast<KeyFrame *>(nullptr)), mbFirstConnection(true), mpParent(nullptr), mbNotErase(false),
...
/**
 * @brief 由于其他的原因,导致当前关键帧观测到的某个地图点被删除(bad==true)了,将该地图点置为nullptr
 * 
...
//      Step 3 遍历每一个当前关键帧的地图点，删除每一个地图点和当前关键帧的联系
        for (auto & mvpMapPoint : mvpMapPoints) {
            if (mvpMapPoint) {
                mvpMapPoint->EraseObservation(this);  // 让与自己有联系的MapPoint删除与自己的联系
            }
        }
...
//                              寻找并更新权值最大的那个共视关系
                                if (w > max) {
                                    pC = pKF;  // 子关键帧
                                    pP = i;  // 目前和子关键帧具有最大权值的关键帧（将来的父关键帧）
                                    max = w;  // 这个最大的权值
...
//      floor向下取整，mfGridElementWidthInv 为每个像素占多少个格子
        const int nMinCellX = max(0, (int) floor((x - float(mnMinX) - factorX) * mfGridElementWidthInv));
        if (nMinCellX >= mnGridCols)
            return vIndices;

//      ceil向上取整
        const int nMaxCellX = min((int) mnGridCols - 1, (int) ceil((x - float(mnMinX) + factorX) * mfGridElementWidthInv));
        if (nMaxCellX < 0)
            return vIndices;

        const int nMinCellY = max(0, (int) floor((y - float(mnMinY) - factorY) * mfGridElementHeightInv));
        if (nMinCellY >= mnGridRows)
            return vIndices;

        const int nMaxCellY = min((int) mnGridRows - 1, (int) ceil((y - float(mnMinY) + factorY) * mfGridElementHeightInv));
        if (nMaxCellY < 0)
            return vIndices;
...
//  判断某个点是否在当前关键帧的图像中
    bool KeyFrame::IsInImage(const float &x, const float &y) const {
        return (x >= float(mnMinX) && x < float(mnMaxX) && y >= float(mnMinY) && y < float(mnMaxY));
    }
 ...
 */
    bool KeyFrame::UnprojectedStereo(int i, Eigen::Vector3f &x3D) {
...
//      Save the id of each MapPoint in this KF, there can be null pointer in the vector
        ...
        for (int i = 0; i < N; ++i) {
            ...
            if (mvpMapPoints[i] && spMP.find(mvpMapPoints[i]) != spMP.end()) // Checks if the element is not null
                mvBackupMapPointsId.push_back((long long)mvpMapPoints[i]->mnId);
...
//      Save the parent id
        ...
        if (mpParent && spKF.find(mpParent) != spKF.end())
            mBackupParentId = (long long)mpParent->mnId;

//      Save the id of the children KF
...
//      Inertial data
        ...
        if (mPrevKF && spKF.find(mPrevKF) != spKF.end())
            mBackupPrevKFId = (long long)mPrevKF->mnId;

        mBackupNextKFId = -1;
        if (mNextKF && spKF.find(mNextKF) != spKF.end())
            mBackupNextKFId = (long long)mNextKF->mnId;
...
//      Connected KeyFrames with him weight
        mConnectedKeyFrameWeights.clear();
...
//        cout << "c";
        ...
        if (u < float(mnMinX) || u > float(mnMaxX))
            return false;
        if (v < float(mnMinY) || v > float(mnMaxY))
            return false;
