...
//          List of keyframes that share the word
            for (auto lit = lKFs.begin(), lend = lKFs.end(); lit != lend;) {
                ...
                if (pMap == pKFi->GetMap()) {
                    lit = lKFs.erase(lit);
//              Don't delete the KF because the class Map clean all the KF when it is destroyed
                } else {
                    ++lit;
                }
            }
...
//              用mBowVec来计算两者的相似度得分
                auto si = float(mpVoc->score(pKF->mBowVec, pKFi->mBowVec));
...
//            Only compare against those keyframes that share enough words
            int maxCommonWords = 0;
            for (auto & lit : lKFsSharingWordsLoop) {
...
//          Only compare against those keyframes that share enough words
            int maxCommonWords = 0;
            for (auto & lit : lKFsSharingWordsMerge) {
...
//      Only compare against those keyframes that share enough words
        ...
        auto minCommonWords = int(maxCommonWords * 0.8);
...
//          遍历当前关键帧bow向量的每个单词
            for (auto vit = pKF->mBowVec.begin(), vend = pKF->mBowVec.end(); vit != vend; ++vit) {  // 拿到当前单词的逆向索引(所有有当前单词的关键帧)
                list<KeyFrame *> &lKFs = mvInvertedFile[vit->first];
//              遍历每个有该单词的关键帧
                for (auto pKFi : lKFs) {
//                    if(spConnectedKF.find(pKFi) != spConnectedKF.end()) {
...
//      Compute similarity score.
        for (auto pKFi : lKFsSharingWords) {
            if (pKFi->mnRelocWords > minCommonWords) {
                nScores++;
                auto si = float(mpVoc->score(F->mBowVec, pKFi->mBowVec));
                pKFi->mRelocScore = si;
                lScoreAndMatch.emplace_back(si, pKFi);
            }
        }
