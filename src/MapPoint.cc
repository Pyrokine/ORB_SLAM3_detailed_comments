 ...
 */
    MapPoint::MapPoint(const Eigen::Vector3f &Pos, KeyFrame *pRefKF, Map *pMap) :
            mnFirstKFid(long(pRefKF->mnId)), mnFirstFrame(long(pRefKF->mnFrameId)), nObs(0), mnTrackReferenceForFrame(0),
            mnLastFrameSeen(0), mnBALocalForKF(0), mnFuseCandidateForKF(0), mnLoopPointForKF(0), mnCorrectedByKF(0),
 ...
 * @brief 构造函数
 */
    MapPoint::MapPoint(const double invDepth, const cv::Point2f& uv_init, KeyFrame *pRefKF, KeyFrame *pHostKF, Map *pMap) :
            mnFirstKFid(long(pRefKF->mnId)), mnFirstFrame(long(pRefKF->mnFrameId)), nObs(0), mnTrackReferenceForFrame(0),
            mnLastFrameSeen(0), mnBALocalForKF(0), mnFuseCandidateForKF(0), mnLoopPointForKF(0), mnCorrectedByKF(0),
            mnCorrectedReference(0), mnBAGlobalForKF(0), mpRefKF(pRefKF), mnVisible(1), mnFound(1), mbBad(false),
            mpReplaced(static_cast<MapPoint *>(nullptr)), mfMinDistance(0), mfMaxDistance(0), mpMap(pMap),
 ...
 */
    MapPoint::MapPoint(const Eigen::Vector3f &Pos, Map *pMap, Frame *pFrame, const int &idxF) :
            mnFirstKFid(-1), mnFirstFrame((long long)pFrame->mnId), nObs(0), mnTrackReferenceForFrame(0), mnLastFrameSeen(0),
...
//          把mObservations指向的内存释放，obs作为局部变量之后自动删除
        ...
        for (auto & ob : obs) {
            ...
            int leftIndex = get<0>(ob.second), rightIndex = get<1>(ob.second);
            if (leftIndex != -1) {
...
//          Replace measurement in keyframe
            ...
            tuple<int, int> indexes = ob.second;
            int leftIndex = get<0>(indexes), rightIndex = get<1>(indexes);
...
//          获得中值
            int median = vDists[int(0.5 * double(N - 1))];
...
//      同时取log线性化
    ...
    [[maybe_unused]] void MapPoint::PrintObservations() {
...
//      1. 备份替代的MP id
        ...
        if (mpReplaced && spMP.find(mpReplaced) != spMP.end())
            mBackupReplacedId = (long long)mpReplaced->mnId;
