...
//                thDepth = mThDepth;
        ...
        if (imageScale != 1.f) {
            int imWidth = int(float(im.cols) / imageScale);
            int imHeight = int(float(im.rows) / imageScale);
...
//        Draw
        ...
        } else if (state == Tracking::OK)// TRACKING
        {
            ...
            mnTrackedVO = 0;
            const float r = 5;
            const int n = int(mvCurrentKeysRight.size());
...
//                    This is a match to a MapPoint in the map
    ...
    void FrameDrawer::DrawTextInfo(cv::Mat &im, int nState, cv::Mat &imText) {
        ...
        if (nState == Tracking::NO_IMAGES_YET)
            ...
            int nKFs = int(mpAtlas->KeyFramesInMap());
            int nMPs = int(mpAtlas->MapPointsInMap());
    ...
    void FrameDrawer::Update(Tracking *pTracker) {
        ...
        if (both) {
            ...
            N = int(mvCurrentKeys.size() + mvCurrentKeysRight.size());
        } else {
...
//        Variables for the new visualization
        ...
        if (pTracker->mLastProcessedState == Tracking::NOT_INITIALIZED) {
            ...
            for (int i = 0; i < N; ++i) {
                ...
                if (pMP) {
                    if (!pTracker->mCurrentFrame.mvbExterior[i]) {
