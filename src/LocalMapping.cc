 ...
 */
    LocalMapping::LocalMapping(System *pSys, Atlas *pAtlas, bool bMonocular, bool bInertial, const string &_strSeqName) :
            mpSystem(pSys), mbMonocular(bMonocular), mbInertial(bInertial), mbResetRequested(false), mbResetRequestedActiveMap(false), mbFinishRequested(false), 
            mbFinished(true), mpAtlas(pAtlas), bInitializing(false), mbAbortBA(false), mbStopped(false), mbStopRequested(false), mbNotStop(false), mbAcceptKeyFrames(true),
...
//                          如果距离大于5厘米，记录当前KF和上一KF时间戳的差，累加到mTInit
                            if (dist > 0.05)
                                mTInit += float(mpCurrentKeyFrame->mTimeStamp - mpCurrentKeyFrame->mPrevKF->mTimeStamp);
...
//                          条件---------1.1、跟踪成功的内点数目大于75-----1.2、并且是单目--或--2.1、跟踪成功的内点数目大于100-----2.2、并且不是单目
                            bool bLarge = ((mpTracker->GetMatchesInners() > 75) && mbMonocular) || ((mpTracker->GetMatchesInners() > 100) && !mbMonocular);
...
//      Step 1：根据相机类型设置不同的观测阈值
        ...
        int borrar = int(mlpRecentAddedMapPoints.size());
...
//              特征点反投影,其实得到的是在各自相机坐标系下的一个非归一化的方向向量,和这个点的反投影射线重合
                Eigen::Vector3f xn1 = pCamera1->unprojectedEig(kp1.pt);
                Eigen::Vector3f xn2 = pCamera2->unprojectedEig(kp2.pt);
...
//              cosParallaxRays>0 && (bStereo1 || bStereo2 || cosParallaxRays<0.9998)表 明视线角正常
//              cosParallaxRays<cosParallaxStereo 表明前后帧视线角比双目视线角大，所以用前后帧三角化而来，反之使用双目的，如果没有双目则跳过
...
//                  如果是双目，用视差角更大的那个双目信息来恢复，直接用已知3D点反投影了
                    goodProj = mpCurrentKeyFrame->UnprojectedStereo(idx1, x3D);
                } else if (bStereo2 && cosParallaxStereo2 < cosParallaxStereo1) {
                    countStereoAttempt++;
                    bPointStereo = true;
//                  如果是双目，用视差角更大的那个双目信息来恢复，直接用已知3D点反投影了
                    goodProj = pKF2->UnprojectedStereo(idx2, x3D);
                } else {
...
//              Step 6：三角化生成3D点成功，构造成MapPoint
                auto *pMP = new MapPoint(x3D, mpCurrentKeyFrame, mpAtlas->GetCurrentMap());
                if (bPointStereo)
                    countStereo++;
...
//      以一级相邻关键帧的共视关系最好的5个相邻关键帧 作为二级相邻关键帧
        for (int i = 0, imax = int(vpTargetKFs.size()); i < imax; ++i) {
...
//          Step 4：该关键帧90%以上的有效地图点被判断为冗余的，则删除该关键帧
            if (float(nRedundantObservations) > redundant_th * float(nMPs)) {
...
//                  关键帧具有前后关键帧
                    if (pKF->mPrevKF && pKF->mNextKF) {
                        const auto t = float(pKF->mNextKF->mTimeStamp - pKF->mPrevKF->mTimeStamp);
//                      下面两个括号里的内容一模一样
 ...
 */
    void LocalMapping::RequestReset() {
        {
            unique_lock<mutex> lock(mMutexReset);
            cout << "LM: Map reset received" << endl;
 ...
 */
    void LocalMapping::RequestResetActiveMap(Map *pMap) {
        {
            unique_lock<mutex> lock(mMutexReset);
            cout << "LM: Active map reset received" << endl;
            mbResetRequestedActiveMap = true;
            mpMapToReset = pMap;
        }
...
//          获得重力坐标系到世界坐标系的旋转矩阵的初值
            ...
            mTInit = float(mpCurrentKeyFrame->mTimeStamp - mFirstTs);
        } else {
...
//              4.1 恢复重力方向与尺度信息
                ...
                mpAtlas->GetCurrentMap()->ApplyScaledRotation(Twg, float(mScale), true);
//              4.2 更新普通帧的位姿，主要是当前帧与上一帧
                mpTracker->UpdateFrameIMU(float(mScale), vpKF[0]->GetImuBias(), mpCurrentKeyFrame);
            }
...
//          7.1 获得这个关键帧的子关键帧
            ...
            const set<KeyFrame *> sChilds = pKF->GetChildren();
...
//      0.4版本这个值为0.00001
        if ((fabs(mScale - 1.f) > 0.002) || !mbMonocular) {
            ...
            mpAtlas->GetCurrentMap()->ApplyScaledRotation(Tgw, float(mScale), true);
            mpTracker->UpdateFrameIMU(float(mScale), mpCurrentKeyFrame->GetImuBias(), mpCurrentKeyFrame);
        }
