...
namespace ORB_SLAM3 {
    ...
    void Viewer::newParameterLoader(Settings *settings) {
        ...
        mImageHeight = float(imSize.height);
        mImageWidth = float(imSize.width);
    ...
    bool Viewer::ParseViewerParamFile(cv::FileStorage &fSettings) {
        ...
        if (!node.empty()) {
            mImageViewerScale = float(node.real());
        }

        node = fSettings["Viewer.ViewpointX"];
        if (!node.empty()) {
            mViewpointX = float(node.real());
        } else {
...
//                cout << "Viewer: step by step" << endl;
            ...
            if (mImageViewerScale != 1.f) {
                auto width = int(float(toShow.cols) * mImageViewerScale);
                auto height = int(float(toShow.rows) * mImageViewerScale);
