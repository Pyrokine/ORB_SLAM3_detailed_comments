...
//      Output welcome message
        ...
        if (mSensor == MONOCULAR)
            ...
            cout << "Unrecognized Sensor Mode" << endl;
            exit(-1);
        }
...
//          保存及加载地图的名字
            ...
            cout << "File version: " << (!node.empty() ? node.string() : "not specified") << endl;
            settings_ = nullptr;
...
//      是否激活回环，默认是开着的
        ...
        cout << "--------------------------------------------------" << endl;
        cout << "Loop Closing: " << (activeLC ? "on" : "off") << endl;

        mStrVocabularyFilePath = strVocFile;

//      ORB SLAM3新加的多地图管理功能，这里加载Atlas标识符
...
//      如果是有imu的传感器类型，设置mbIsInertial = true，以后的跟踪和预积分将和这个标志有关
        if (mSensor == IMU_STEREO || mSensor == IMU_MONOCULAR || mSensor == IMU_RGBD) {
            cout << "--------------------------------------------------" << endl;
            cout << "Configuring Inertial Sensor" << endl;
...
//      创建用于显示帧和地图的类，由Viewer调用
        cout << "--------------------------------------------------" << endl;
        cout << "Initializing Frame Drawer..." << endl;
        mpFrameDrawer = new FrameDrawer(mpAtlas);

        cout << "--------------------------------------------------" << endl;
        cout << "Initializing Map Drawer..." << endl;
        mpMapDrawer = new MapDrawer(mpAtlas, strSettingsFile, settings_);

//      创建跟踪线程（主线程）,不会立刻开启,会在对图像和imu预处理后在main主线程种执行
        cout << "--------------------------------------------------" << endl;
        cout << "Initializing Tracking..." << endl;
...
//      设置最远3D地图点的深度值，如果超过阈值，说明可能三角化不太准确，丢弃
        ...
        else if (mpLocalMapper->mThFarPoints < 0) {
            cout << "thFarPoints should be a positive value" << endl;
            exit(-1);
        }
        else {
            cout << "Far points will not be filtered" << endl;
            mpLocalMapper->mbFarPoints = false;
        }

//      创建并开启闭环线程
        cout << "--------------------------------------------------" << endl;
        cout << "Initializing LoopClosing..." << endl;
...
//          for point cloud PCMResolution
            float PCMResolution = fsSettings["PointCloudMapping.PCMResolution"];
            float PCMMeanK = fsSettings["PointCloudMapping.PCMMeanK"];
            float PCMThresh = fsSettings["PointCloudMapping.PCMThresh"];
            cout << "PointCloudMapping parameters:" << endl;
            cout << "PCMResolution: " << PCMResolution << ", PCMMeanK: " << PCMMeanK << ", PCMThresh: " << PCMThresh << endl;

            mpPointCloudMapping = new PointCloudMapping(PCMResolution, PCMMeanK, PCMThresh);
...
//        std::cout << "start GrabImageStereo" << std::endl;
        Sophus::SE3f Tcw = mpTracker->GrabImageStereo(imLeftToFeed, imRightToFeed, timestamp, std::move(filename));
...
//      Check reset
        ...
        Sophus::SE3f Tcw = mpTracker->GrabImageRGBD(imToFeed, imDepthToFeed, timestamp, std::move(filename));
...
//        }
        ...
        for (Map *pMap: vpMaps) {
            ...
            if (pMap->GetAllKeyFrames().size() > numMaxKFs) {
                numMaxKFs = int(pMap->GetAllKeyFrames().size());
                pBiggerMap = pMap;
            }
        }
...
//  校验词典文件，哈希出一个值，两个哈希值一样表示是同一文件
    string System::CalculateCheckSum(const string& filename, int type) {
        ...
        MD5_Init(&md5Context);
        while (auto count = int(f.readsome(buffer, sizeof(buffer)))) {
