...
//      Constructor for stereo cameras.
        Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp, ORBExtractor *extractorLeft, ORBExtractor *extractorRight, ORBVocabulary *voc, cv::Mat &K,
              cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera *pCamera, Frame *pPrevF = static_cast<Frame *>(nullptr), const IMU::Calib &ImuCalib = IMU::Calib());

//      Constructor for RGB-D cameras.
        Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, ORBExtractor *extractor, ORBVocabulary *voc, cv::Mat &K, cv::Mat &distCoef, const float &bf,
              const float &thDepth, GeometricCamera *pCamera, Frame *pPrevF = static_cast<Frame *>(nullptr), const IMU::Calib &ImuCalib = IMU::Calib());
...
//      Constructor for Monocular cameras.
        Frame(const cv::Mat &imGray, const double &timeStamp, ORBExtractor *extractor, ORBVocabulary *voc, GeometricCamera *pCamera, cv::Mat &distCoef, const float &bf,
              const float &thDepth, Frame *pPrevF = static_cast<Frame *>(nullptr), const IMU::Calib &ImuCalib = IMU::Calib());
...
//      Compute the cell of a keypoint (return false if outside the grid)
        ...
        [[nodiscard]] vector<size_t> GetFeaturesInArea(const float &x, const float &y, const float &r, int minLevel = -1, int maxLevel = -1, bool bRight = false) const;
...
//      Back-projects a keypoint (if stereo/depth info available) into 3D world coordinates.
        bool UnprojectedStereo(const int &i, Eigen::Vector3f &x3D);
...
//      Returns inverse of rotation
        ...
        [[nodiscard]] inline Sophus::SE3<float> GetPose() const {
//          TODO: can the Frame pose be accessed from several threads? should this be protected somehow?
            return mTcw;
        }

        [[nodiscard]] inline Eigen::Matrix3f GetRwc() const {
            return mRwc;
        }

        [[nodiscard]] inline Eigen::Vector3f GetOw() const {
            return mOw;
        }

        [[nodiscard]] inline bool HasPose() const {
            return mbHasPose;
        }

        [[nodiscard]] inline bool HasVelocity() const {
...
//      MapPoints associated to key points, nullptr pointer if no association.
//      Flag to identify outlier associations.
        std::vector<bool> mvbExterior;
        int mnCloseMPs{};
...
//      Undistorted key points given OpenCV distortion parameters.
...
//      Number of KeyPoints extracted in the left and right images
        int Nleft{}, Nright{};
//      Number of Non Lapping Key Points
...
//      Grid for the right image
        ...
        Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp, ORBExtractor *extractorLeft, ORBExtractor *extractorRight, ORBVocabulary *voc,
              cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera *pCamera, GeometricCamera *pCamera2, Sophus::SE3f &Tlr,
...
//      Stereo fisheye
        ...
        Eigen::Vector3f UnprojectedStereoFishEye(const int &i);
        ...
        void PrintPointDistribution() {
            ...
            for (int i = 0; i < N; i++) {
                if (mvpMapPoints[i] && !mvbExterior[i]) {
                    if (i < Nlim) left++;
                    else right++;
                }
            }
