...
//      Compute the Fundamental matrix between KF1 and KF2
        [[maybe_unused]] static Eigen::Matrix3f ComputeF12(KeyFrame *&pKF1, KeyFrame *&pKF2);
...
//      Triangulate point with KF1 and KF2
        ...
        [[maybe_unused]] static bool CheckMatrices(const cv::Mat &cvMat, const Eigen::Matrix<float, rows, cols> &eigMat) {
...
//          std::cout << cvMat.cols - cols << cvMat.rows - rows << std::endl;
        ...
        [[maybe_unused]] static bool CheckMatrices(const Eigen::Matrix<T, rows, cols> &eigMat1, const Eigen::Matrix<T, rows, cols> &eigMat2) {
            const float epsilon = 1e-3;
            for (int i = 0; i < rows; i++)
                for (int j = 0; j < cols; j++)
