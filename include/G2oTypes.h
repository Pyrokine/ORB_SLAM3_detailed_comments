...
namespace ORB_SLAM3 {
    ...
    [[maybe_unused]] typedef Eigen::Matrix<double, 12, 1> Vector12d;
    typedef Eigen::Matrix<double, 15, 1> Vector15d;
    [[maybe_unused]] typedef Eigen::Matrix<double, 12, 12> Matrix12d;
...
//  相关节点中使用，存放的是imu与cam的内外参
    ...
    public:
        ...
        [[nodiscard]] Eigen::Vector3d ProjectStereo(const Eigen::Vector3d &Xw, int cam_idx = 0) const;  // Stereo
...
//  重力方向
    ...
    public:
        ...
        explicit GDirection(Eigen::Matrix3d pRwg) : Rwg(std::move(pRwg)) {}
 ...
 */
    ...
    public:
        ...
        explicit VertexGDir(Eigen::Matrix3d pRwg) {
            setEstimate(GDirection(std::move(pRwg)));
        }
 ...
 */
    ...
    public:
        ...
        [[maybe_unused]] VertexInvDepth(double invDepth, double u, double v, KeyFrame *pHostKF) {
...
//      计算重投影误差
        ...
        [[maybe_unused]] Eigen::Matrix<double, 2, 9> GetJacobian() {
            linearizeOplus();
...
//      可以理解为像素的不确定性给旋转平移跟三维点带来了多大的不确定性
        [[maybe_unused]] Eigen::Matrix<double, 9, 9> GetHessian() {
            linearizeOplus();
            Eigen::Matrix<double, 2, 9> J;
            J.block<2, 3>(0, 0) = _jacobianOplusXi;
            J.block<2, 6>(0, 3) = _jacobianOplusXj;
 ...
 */
    ...
    public:
        ...
        explicit EdgeMonoOnlyPose(const Eigen::Vector3f &Xw_, int cam_idx_ = 0) : Xw(Xw_.cast<double>()), cam_idx(cam_idx_) {}
...
//      代码中都是0
        ...
        [[maybe_unused]] Eigen::Matrix<double, 3, 9> GetJacobian() {
            linearizeOplus();
 ...
 */
    ...
    public:
        ...
        explicit EdgeStereoOnlyPose(const Eigen::Vector3f &Xw_, int cam_idx_ = 0) : Xw(Xw_.cast<double>()), cam_idx(cam_idx_) {}
...
//      没用
        [[maybe_unused]] Eigen::Matrix<double, 18, 18> GetHessianNoPose1() {
            linearizeOplus();
...
//      关于pose1与2 的旋转平移速度，以及之间的偏置，还有重力方向与尺度的信息矩阵
        [[maybe_unused]] Eigen::Matrix<double, 27, 27> GetHessian() {
            linearizeOplus();
...
//      与上面摆放不同
        [[maybe_unused]] Eigen::Matrix<double, 27, 27> GetHessian2() {
            linearizeOplus();
            Eigen::Matrix<double, 9, 27> J;
...
//      关于偏置，重力方向与尺度的信息矩阵
        [[maybe_unused]] Eigen::Matrix<double, 9, 9> GetHessian3() {
            linearizeOplus();
            Eigen::Matrix<double, 9, 9> J;
            J.block<9, 3>(0, 0) = _jacobianOplus[2];
            J.block<9, 3>(0, 3) = _jacobianOplus[3];
...
//      下面的没有用到，其实也是获取状态的信息矩阵
        [[maybe_unused]] Eigen::Matrix<double, 1, 1> GetHessianScale() {
            linearizeOplus();
            Eigen::Matrix<double, 9, 1> J = _jacobianOplus[7];
            return J.transpose() * information() * J;
        }

        [[maybe_unused]] Eigen::Matrix<double, 3, 3> GetHessianBiasGyro() {
            linearizeOplus();
            Eigen::Matrix<double, 9, 3> J = _jacobianOplus[2];
            return J.transpose() * information() * J;
        }

        [[maybe_unused]] Eigen::Matrix<double, 3, 3> GetHessianBiasAcc() {
            linearizeOplus();
            Eigen::Matrix<double, 9, 3> J = _jacobianOplus[3];
            return J.transpose() * information() * J;
        }

        [[maybe_unused]] Eigen::Matrix<double, 2, 2> GetHessianGDir() {
            linearizeOplus();
 ...
 */
    ...
    public:
        ...
        ConstraintPoseImu(
                Eigen::Matrix3d Rwb_, Eigen::Vector3d twb_, Eigen::Vector3d vwb_,
                Eigen::Vector3d bg_, Eigen::Vector3d ba_, Matrix15d H_)
                : Rwb(std::move(Rwb_)), twb(std::move(twb_)), vwb(std::move(vwb_)), bg(std::move(bg_)), ba(std::move(ba_)), H(std::move(H_)) {
            H = (H + H) / 2;  // 对称化
//          令特征值小的位置变为0
 ...
 */
    ...
    public:
        ...
        [[maybe_unused]] Eigen::Matrix<double, 9, 9> GetHessianNoPose() {
            linearizeOplus();
 ...
 */
    ...
    public:
        ...
        void computeError() override {
            const auto *VA = dynamic_cast<const VertexAccBias *>(_vertices[0]);
            _error = bprior - VA->estimate();
        }

        void linearizeOplus() override;

        [[maybe_unused]] Eigen::Matrix<double, 3, 3> GetHessian() {
            linearizeOplus();
            return _jacobianOplusXi.transpose() * information() * _jacobianOplusXi;
        }
 ...
 */
    ...
    public:
        ...
        void computeError() override {
            const auto *VG = dynamic_cast<const VertexGyroBias *>(_vertices[0]);
 ...
 */
    ...
    public:
        ...
        void computeError() override {
            const auto *VPi = dynamic_cast<const VertexPose4DoF *>(_vertices[0]);
            const auto *VPj = dynamic_cast<const VertexPose4DoF *>(_vertices[1]);
