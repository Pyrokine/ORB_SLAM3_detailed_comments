...
namespace ORB_SLAM3 {
    ...
    public:
        ...
        KannalaBrandt8(const std::vector<float>& _vParameters, const float _precision) : GeometricCamera(_vParameters), precision(_precision), mvLappingArea(2, 0) {
            assert(mvParameters.size() == 8);
            mnId = nNextId++;
            mnType = CAM_FISHEYE;
        }
        ...
        Eigen::Vector3f unprojectedEig(const cv::Point2f &p2D) override;

        cv::Point3f unprojected(const cv::Point2f &p2D) override;

        Eigen::Matrix<double, 2, 3> projectedJac(const Eigen::Vector3d &v3D) override;
        ...
        bool epipolarConstrain(GeometricCamera *pCamera2, const cv::KeyPoint &kp1, const cv::KeyPoint &kp2, const Eigen::Matrix3f &R12, const Eigen::Vector3f &t12,
                               float sigmaLevel, float unc) override;

        float TriangulateMatches(GeometricCamera *pCamera2, const cv::KeyPoint &kp1, const cv::KeyPoint &kp2, const Eigen::Matrix3f &R12, const Eigen::Vector3f &t12,
                                 float sigmaLevel, float unc, Eigen::Vector3f &p3D);

        std::vector<int> mvLappingArea;

        bool matchAndTriangulate(const cv::KeyPoint &kp1, const cv::KeyPoint &kp2, GeometricCamera *pOther, Sophus::SE3f &Tcw1, Sophus::SE3f &Tcw2,
                                 float sigmaLevel1, float sigmaLevel2, Eigen::Vector3f &x3DTriangulated) override;
        ...
        [[nodiscard]] float GetPrecision() const { return precision; }
...
//      [fx, fy, cx, cy, k0, k1, k2, k3]
        ...
        static void Triangulate(const cv::Point2f &p1, const cv::Point2f &p2, const Eigen::Matrix<float, 3, 4> &Tcw1, const Eigen::Matrix<float, 3, 4> &Tcw2, Eigen::Vector3f &x3D);
    };
}
